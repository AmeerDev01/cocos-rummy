    ...
    execute: function () {
      ...
      _export("ThorV2_Roller", ThorV2_Roller = (_dec = ccclass('ThorV2_Roller'), _dec(_class = class ThorV2_Roller extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        newIcon() {
          ...
          var position = this.geticonBirthPos();
          icon.getNode().setPosition(position);
          return icon;
        }
        /**计算两个索引的差值 */
        ...
        fillIconData() {
          var emptyStartIndex = this.getFirstEmptyIconIndex(); // 计算从空图标位置之后有几个非空图标

          var notEmptyCount = 0;

          for (var i = emptyStartIndex + 1; i < this.iconArr.length; i++) {
          ...
          var addIconCount = this.showIconCount - this.getFirstEmptyIconIndex();

          if (addIconCount > notEmptyCount) {
            for (var _i = 0; _i < addIconCount - notEmptyCount; _i++) {
              this.addNewIcon();
            }
          }
        }
        ...
        getOdds(index, id, rollerLaunchResult) {
          ...
          var rates = rollerLaunchResult.dl.si[0].otherGameAttributesVo.rates;
        ...
        clearIcon(indexs, roundIndex, done) {
          var timeScale = this.isSpeed ? (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
        ...
        changeRollerHeight(gameType) {
          ...
          var rollerWidth = (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
          ...
          var uiTransform = this.propertyNode.props_icon_list.getComponent(UITransform);
        ...
        clearAllIcon(isDelay, done) {
          ...
          var columnDelayTime = isDelay ? this.columnIndex * 0.01 : 0;
          var count = 0;
          this.scheduleOnce(() => {
            var icons = this.iconArr.filter(v => v);
        ...
        addIcon(done) {
          ...
          var isDelay = !this.isSpeed;
          var startIndex = this.getFirstEmptyIconIndex();
          ...
          for (var i = startIndex; i < this.showIconCount; i++) {
            fillIndexs.push(i);

            var _loop = function _loop(j) {
              var element = _this.iconArr[j];
            ...
            for (var j = startIndex + 1; j < this.iconArr.length; j++) {
              var _ret = _loop(j);
          ...
          var time = isDelay ? this.columnIndex * 0.05 : 0;
          var count = 0;
          /**所有动画播放结束之后的处理 */
          ...
          this.scheduleOnce(() => {
            fillIndexs.forEach((fillIndex, i) => {
              ...
              if (iconObj.isOddsIcon()) {
                var oddsValue = this.getOdds(nextIndex, iconObj.getId(), this.rollerLaunchResult);
        ...
        deleteWaitPlayAnimationIcon(id) {
          for (var i = 0; i < this.waitPlayAnimationIcon.length; i++) {
            var element = this.waitPlayAnimationIcon[i];
        ...
        addNewIcon() {
          var index = -1;
          var newIcon = this.newIcon();

          for (var i = this.showIconCount; i < this.iconArr.length; i++) {
            var element = this.iconArr[i];
        ...
        dropIcon(orderIndex, icon, curIndex, nextindex, done, isDelay) {
          if (isDelay === void 0) {
            isDelay = false;
          }

          var rowDelayTime = orderIndex * 0.02;
          var delayTime = isDelay ? this.columnIndex * 0.1 + rowDelayTime : 0;
          var pos = nextindex === -1 ? this.getIconHidePos(icon.getPosition()) : this.getIconPosByIndex(nextindex);
          var dropTime = this.getIconDropTime();
          ...
          var tw = tween(icon.getNode()).tag(TWEEN_ICON_TAG).delay(delayTime).to(dropTime, {
        ...
        geticonBirthPos() {
          var y = this.getIconContainerAnchorPos() + (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
          }), config) : config).normalRollOption.singleRollerHeight;
          var pos = new Vec3();
          pos.y += y;
        ...
        getIconHidePos(startPos) {
          ...
          var iconListUI = this.propertyNode.props_icon_list.getComponent(UITransform);
          var pos = new Vec3(startPos.x, startPos.y);
        ...
        playAmount(indexs, amount) {
          ...
          var startIndex = 0;
          var middleIndex = Math.floor(this.showIconCount / 2);
          var indexsTemp = indexs.map(v => v + '');

          var _loop2 = function _loop2() {
            var leftIndex = middleIndex - startIndex;
            var rightIndex = middleIndex + startIndex;

            if (leftIndex < 0 || rightIndex >= _this2.showIconCount) {
              return "break";
            }

            if (indexsTemp.find(v => v === leftIndex + '')) {
              _this2.iconArr[leftIndex].playAmount(_this2.propertyNode.props_number, amount);

              return "break";
            }

            if (indexsTemp.find(v => v === rightIndex + '')) {
              _this2.iconArr[rightIndex].playAmount(_this2.propertyNode.props_number, amount);
        ...
        getMiddleOrder(indexs) {
          ...
          var middleColumnIndex = (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
          ...
          var order = Math.abs(middleColumnIndex - this.columnIndex) * 5;
          var startIndex = 0;
          var middleIndex = Math.floor(this.showIconCount / 2);
          var indexsTemp = indexs.map(v => v + '');
        ...
        playLightningAnimation(an, endPos) {
          var topNode = (_crd && mainViewModel === void 0 ? (_reportPossibleCrUseOfmainViewModel({
            error: Error()
          ...
          var worldPos = this.toWorldSpaceAR(this.propertyNode.props_icon_list, endPos);
        ...
        playOddsAnimation(done) {
          ...
          for (var _i2 = 0; _i2 < this.showIconCount; _i2++) {
            var element = this.iconArr[_i2];
        ...
        playLabelAnimation(parentNode, endPos, done) {
          var labelNodes = this.getOddsLabelNodes();
...
//# sourceMappingURL=694aeb0fd31fe68f87cfb175ecc28ae8799346a4.js.map