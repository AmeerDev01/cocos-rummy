  ...
  return {
    ...
    execute: function () {
      ...
      _export("ObjectPool", ObjectPool = class ObjectPool {
        ...
        initConfig(bulletConfig, batteryConfig, commonConfig, fishConfig, textureConfig, fishHitLabelConfig, fishLabelFntConfig, fishNetConfig, fishGoldConfig, activeConfig, fishFntResConfig, batteryManager, sourceManage, scaleRatio, rootNode) {
        ...
        init(parent) {
          ...
          for (var k in ObjectClassType) {
            var v = ObjectClassType[k];
            if (!v) continue;
        ...
        registerObjectCreater(cType, creater) {
          var item = this.objectClassList[cType];
        ...
        newSomeObject(cType, dataID, number) {
          var cItem = this.objectClassList[cType];
          ...
          if (cItem && cItem.creater) {
            for (var i = 1; i <= number; i++) {
              ...
              var creater = cItem.creater;
              var newObj = creater.create(_info);
              ...
              if (cType === ObjectClassType.type_bullet) {
                ...
                if (ObjectClassType.type_fish_net === cType) {
                  var fishNet = newObj;
                  fishNet.init(_info, this.fishNetConfig, this.sourceManage, this.rootNode);
                } else if (ObjectClassType.type_fish_gold === cType) {
                  var fishGold = newObj;
                  fishGold.init(_info, this.sourceManage, this.fishGoldConfig, this.textureConfig, this.activeConfig, this.commonConfig, this.rootNode);
                } else if (ObjectClassType.type_fish_fnt === cType) {
                  var fishLabelFnt = newObj;
                  fishLabelFnt.init(_info, this.sourceManage, this.fishLabelFntManager, this.fishLabelFntConfig, this.fishFntResConfig, this.activeConfig, this.commonConfig, this.rootNode);
                }
        ...
        getObject(cType, dataID) {
          ...
          if (cItem) {
            for (var k in cItem.object_lst) {
              var v = cItem.object_lst[k];
              if (!v) continue;
              var buse = v.getUseState();
              var dID = v.getDataId();
...
//# sourceMappingURL=0806cb5a67cc698268343fa02cd1f4c37b4770bc.js.map