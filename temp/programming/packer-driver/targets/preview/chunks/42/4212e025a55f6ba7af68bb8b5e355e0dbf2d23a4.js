  ...
  return {
    ...
    execute: function () {
      ...
      _export("Crash_Game_contentLayer", Crash_Game_contentLayer = (_dec = ccclass('Game_contentLayer'), _dec(_class = (_class2 = class Crash_Game_contentLayer extends (_crd && Crash_ViewBase === void 0 ? (_reportPossibleCrUseOfCrash_ViewBase({
        ...
        init() {
          var LotteryData = (_crd && Crash_DataManager === void 0 ? (_reportPossibleCrUseOfCrash_DataManager({
            error: Error()
          }), Crash_DataManager) : Crash_DataManager).getInstance().getLotteryData();
          var seconds = (LotteryData.seconds / 1000).toFixed(2);
          var total = LotteryData.drawPrizeNum || 10;
          this._nodes['spine'].active = true;
          var times = Number(seconds) > 3 ? 3 : Number(seconds);
        ...
        playAnimation() {
          ...
          var animationComponent = this._nodes['spine'].getComponent(Animation);
          ...
          var animationState = animationComponent.getState('hide');
        ...
        setDrawProgress(value) {
          ...
          this.drawBezierCurve(this.drawProgress, point => {
            var angle = Math.atan2(point.y, point.x); // console.log(angle);
        ...
        drawBezierCurve(progress, onPointUpdated) {
          var graphics = this._nodes['BezierCurve'].getComponent(Graphics); // 清除之前的绘制内容
          ...
          var startPoint = new Vec2(-70, 0);
          var controlPoint1 = new Vec2(700, 50); // 更靠近起点的控制点

          var controlPoint2 = new Vec2(800, 80); // 更靠近终点的控制点

          var endPoint = new Vec2(800, 205); // 计算贝塞尔曲线上的点

          var points = [];

          for (var t = 0; t <= progress; t += 0.01) {
            var _point = this.calculateBezierPoint(t, startPoint, controlPoint1, controlPoint2, endPoint);

            points.push(_point);
            onPointUpdated(_point);
          } // 绘制贝塞尔曲线


          graphics.moveTo(startPoint.x, startPoint.y);

          for (var _point2 of points) {
        ...
        update(deltaTime) {
          ...
          var data = (_crd && Crash_DataManager === void 0 ? (_reportPossibleCrUseOfCrash_DataManager({
            error: Error()
          }), Crash_DataManager) : Crash_DataManager).getInstance();
          if (!data.getCrashStageVo()) return;

          if (data.getCrashStageVo().gameType === 3) {
            var winGold = data.getUserInfo().getUserInfo();
            var WIN = this._nodes['WIN'];
            ...
            var _winGold = data.getUserInfo().getUserInfo();

            delete _winGold.winGold;
...
//# sourceMappingURL=4212e025a55f6ba7af68bb8b5e355e0dbf2d23a4.js.map