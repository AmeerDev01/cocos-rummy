    ...
    execute: function () {
      ...
      _export("FishManager", FishManager = class FishManager {
        ...
        freshFish(dt) {
          ...
          for (var i = 0; i < this.fishDataList.length; i++) {
            var v = this.fishDataList[i];
            ...
            if (v.run_t >= 0) {
              ...
              var fObj = new (_crd && Fish === void 0 ? (_reportPossibleCrUseOfFish({
                error: Error()
        ...
        removeFish(objID) {
          var fObj = this.aliveFishList.get(objID);
        ...
        shotFish(bbx) {
          for (var [k, v] of this.aliveFishBBList.entries()) {
            if (v) {
              for (var m in bbx) {
                // 子弹的包围盒坐标
                var n = bbx[m];
                ...
                for (var i in v) {
                  // 鱼的包围盒坐标
                  var j = v[i];
                  var fish = this.aliveFishList.get(k);
                  var fishpos = fish.getCurrentPos(); // 计算两点间的距离

                  var dis = sqrt(pow(j.a - n.a, 2) + pow(j.b - n.b, 2)); // console.log(`bullet box ${n.a}, ${n.b}, ${n.r}, fish box ${j.a}, ${j.b}, ${j.r}, fish pos ${fishpos.x}, ${fishpos.y}, angle ${fish.getCurrentAngle()}, dis ${dis}`);
        ...
        getFishIdByPoint(pos) {
          ...
          for (var [k, v] of this.aliveFishList.entries()) {
...
//# sourceMappingURL=4233496a5b8b652df10603faa30eeb47907831a6.js.map