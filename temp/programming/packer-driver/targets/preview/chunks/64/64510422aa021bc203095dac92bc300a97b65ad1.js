      ...
      _export("Hall_SignIn", Hall_SignIn = (_dec = ccclass('Hall_SignIn'), _dec(_class = class Hall_SignIn extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        bindEvent() {
          ...
          this.propertyNode.props_btn_menerima.node.on(Node.EventType.TOUCH_END, () => {
            var weekType = this.getSignInWeekType(); // 签到防抖
        ...
        updatePositionStatus(signInDataType) {
          var weekType = this.getWeekType(signInDataType.day);
          var type = signInDataType.status;
          var gold = signInDataType.gold;
          var signNode = this.getSignInNode(weekType);
          signNode.active = true;
          var goldNode = signNode.getChildByName("label_gold");
          var unclaimedNode = signNode.getChildByName("spr_unclaimed");
          var alreadyNode = signNode.getChildByName("spr_already");
        ...
        playSignInAnimaton(weekType) {
          ...
          var signInData = this.props.signInDatas.find(v => v.day === this.getDay(weekType));
          var gold = signInData ? signInData.gold : 0;
        ...
        init(datas) {
          // 可以签到的某一天
          var signDay = datas.find(v => v.tap === 1);
          datas.forEach(v => {
          ...
          if (signDay) {
            ...
            var lastSignDay = [...datas].reverse().find(v => v.status === 1);
        ...
        getSignInWeekType() {
          var signDay = this.props.signInDatas.find(v => v.tap === 1);
        ...
        useProps(key, value) {
          ...
          if (key === 'signInDatas') {
            var newData = value.cur;

            if (newData.length > 0) {
              this.init(newData);
            }
          } else if (key === 'signSuccess' && value.cur) {
            var signType = this.getSignInWeekType();
...
//# sourceMappingURL=64510422aa021bc203095dac92bc300a97b65ad1.js.map