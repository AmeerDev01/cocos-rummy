    ...
    execute: function () {
      ...
      _export("Fish", Fish = class Fish {
        ...
        init(objID, fishID, curveID, isSkew, runSeconds, startPos, isReserve, angle, userInfo, sceneID) {
          ...
          for (var i = 0; i < this.cvConfig.data.length; i++) {
            var cveinfo = instantiate(this.cvConfig.data[i]); // if (cveinfo.type !== CURVE_TYPE.pbl && cveinfo.type !== CURVE_TYPE.ccve) {
          ...
          if (useTestLabel) {
            ...
            var label = this.tesLabelObj.addComponent(Label);
        ...
        update(dt) {
          ...
          this.runedSeconds = this.runedSeconds + dt;
          var res = this.curveObject.getPosAndAngleByTime(this.runedSeconds);
          ...
          if (isDrawBondingBox) {
            var graphics = this.draw.getComponent(Graphics);
            graphics.clear();

            for (var key in this.bbConfig.data) {
              var v = this.bbConfig.data[key];
              if (!v) continue;
        ...
        isShotFish(cirLst) {
          ...
          var ranti = deg(360 - this.currentAngle);

          if (this.currentPos) {
            for (var i = 0; i < cirLst.length; i++) {
              var v = cirLst[i];

              for (var m in this.bbConfig.data) {
                var n = this.bbConfig.data[m];
                var newA = (n.a * cos(ranti) + this.skewMod * n.b * sin(ranti)) * this.scaleRatio;
                var newB = (this.skewMod * n.b * cos(ranti) - n.a * sin(ranti)) * this.scaleRatio;
                var dist = sqrt(pow(newA + this.currentPos.x - v.a, 2) + pow(newB + this.currentPos.y - v.b, 2));
        ...
        getCurrentBBX() {
          var bbx = instantiate(this.bbConfig.data); // let ranti = (this.currentAngle / pia) * pi
          // 这里 360 - this.currentAngle，是在碰撞检测的时候有时候距离有点远

          var ranti = deg(360 - this.currentAngle);

          for (var k in this.bbConfig.data) {
            var v = this.bbConfig.data[k];
        ...
        isPointInFish(pos) {
          if (this.currentPos) {
            ...
            for (var m in this.bbConfig.data) {
              ...
              var newA = (n.a * cos(ranti) + this.skewMod * n.b * sin(ranti)) * this.scaleRatio;
              var newB = (this.skewMod * n.b * cos(ranti) - n.a * sin(ranti)) * this.scaleRatio;
              var dist = sqrt(pow(newA + this.currentPos.x - pos.x, 2) + pow(newB + this.currentPos.y - pos.y, 2));
        ...
        fishStart() {
          ...
          this.showDeadSpr(false);
          this.runStartEff();
          var res = this.curveObject.getPosAndAngleByTime(0.00001);
          this.currentPos = res[0];
          this.currentAngle = res[1];
          this.setFishPosAndAngle();
        }
        ...
        showFishEff() {
          ...
          for (var k = 0; k < this.aliveList.length; k++) {
            _loop();
          }
        }
        ...
        createFishItem(data, lst, busepit) {
          var len = data.view_inf.length - 1;

          for (var i = 0; i <= len; i++) {
            var src_cfg = this.textureConfig[data.view_inf[i].src_id];
            ...
            if (src_cfg.stype === 1) {
              ...
              //粒子
              if (busepit) {
                var pit_obj = (_crd && createParticleSystemNode === void 0 ? (_reportPossibleCrUseOfcreateParticleSystemNode({
                  error: Error()
        ...
        drawFishCurve(startPos) {
          var tmpCurveObj = instantiate(this.curveObject);
          this.curveDrawObj = new Node();
          var graphics = this.addGraphics(this.curveDrawObj);
          this.parantNode.insertChild(this.curveDrawObj, 100000000); // let spos = instantiate(startPos);

          var spos = (_crd && toNodeSpaceAR === void 0 ? (_reportPossibleCrUseOftoNodeSpaceAR({
            error: Error()
          }), toNodeSpaceAR) : toNodeSpaceAR)(this.rootNode, startPos);
          var drawDt = 0;

          while (!tmpCurveObj.isCurveEnd()) {
            var epos = tmpCurveObj.getPosAndSlopeByTime(drawDt)[0]; // console.log(epos.x, epos.y);
        ...
        runStartEff() {
          for (var k = 0; k < this.aliveList.length; k++) {
            ...
            if (v.stype === 1) {
              ...
              //todo
              var srcConfig = this.textureConfig[v.src_id];
              var pit_obj = (_crd && createParticleSystemNode === void 0 ? (_reportPossibleCrUseOfcreateParticleSystemNode({
                error: Error()
          ...
          for (var i = 0; i <= this.fishConfig.alive.skl_id_lst.length; i++) {
        ...
        runDeadEff() {
          this.deathList.forEach(v => {
            if (v.stype === 1) {
              ...
              //todo
              var tc = this.textureConfig[v.src_id];
          ...
          var fntID = 1;
          var btObj = this.batteryManager.getBatteryByPlayerId(this.userInfo.player_id);

          if (btObj) {
            var fnt = btObj.getDEfntConfig();
          ...
          for (var i = 0; i < this.fishConfig.death.skl_id_lst.length; i++) {
...
//# sourceMappingURL=ec6f5091375289e13ae14a817b47806422c5e709.js.map