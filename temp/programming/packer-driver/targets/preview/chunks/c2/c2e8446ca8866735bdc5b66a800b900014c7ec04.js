    ...
    execute: function () {
      ...
      _export("Domino_Mine", Domino_Mine = (_dec = ccclass('Domino_Mine'), _dec(_class = class Domino_Mine extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        getSiblingIndex(cardItem) {
          for (var i = this.handCardViewModelList.length - 1; i >= 0; i--) {
            var element = this.handCardViewModelList[i];
            ...
            if (cardItem.downFace > element.comp.props.downFace) {
              ...
              for (var j = i; j >= 0; j--) {
                var elementJ = this.handCardViewModelList[j];
        ...
        deal(done, isAnime) {
          ...
          var cardItem = this.props.cardIList[this.dealIndex++];
        ...
        genCardView(cardItem, done, isAnime) {
          if (isAnime === void 0) {
            isAnime = true;
          }

          if (!cardItem) return;
          var siblingIndex = this.getSiblingIndex(cardItem);
          ...
          cardViewModel.setComplateCallback(() => {
            var position = this.propertyNode.props_Layout_cards_handCards.getComponent(UITransform).convertToWorldSpaceAR(cardViewModel.comp.node.getPosition());
          ...
          this.handCardViewModelList.sort((s1, s2) => {
            var r = s1.comp.props.downFace - s2.comp.props.downFace;
        ...
        updateCardState(out) {
          var outLength = this.props.outCardItem.length;
          ...
          if (out && outLength === 0) {
            ...
            if (out) {
              ...
              var isCanOut = this.handCardViewModelList.find(v => v.isBright());
        ...
        updateMissNumber(values) {
          ...
          this.propertyNode.props_Layout_cards_misNum.children.forEach(node => {
            var value = values.find(value => node.name === 'props_spr_cards_misNum-00' + value);
        ...
        updateWinloss(value) {
          ...
          var initPos = new Vec3(labelNode.position.x, labelNode.position.y);
...
//# sourceMappingURL=c2e8446ca8866735bdc5b66a800b900014c7ec04.js.map