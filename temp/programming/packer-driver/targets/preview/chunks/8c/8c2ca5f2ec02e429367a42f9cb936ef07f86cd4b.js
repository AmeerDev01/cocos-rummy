System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  function rotatinPoint(x, y, degree) {
    var radian = degree / (_crd && pia === void 0 ? (_reportPossibleCrUseOfpia({
      error: Error()
  ...
  function getCurPoint(t, prePos, item) {
    ...
    var pos = (_crd && frule === void 0 ? (_reportPossibleCrUseOffrule({
      error: Error()
  ...
  function getPostionByTime(t, lst, rot, spos, startIndex) {
    ...
    for (var i = startIndex; i <= len; i++) {
      ...
      if (t >= v.t1 && t < v.t2) {
        var ep = i > 0 ? lst[i - 1].ep : undefined;
        var pos = getCurPoint(t, ep, v);
        curPos.x = pos[0];
        curPos.y = pos[1];
    ...
    execute: function () {
      ...
      _export("ActionCurve", ActionCurve = class ActionCurve {
        ...
        addCurve(cveinfo) {
          ...
          var len = this.funcLst.length - 1;

          if (len < 0) {
            fitem.t1 = 0;
            fitem.t2 = fitem.t1 + fitem.time;
            var pos = getCurPoint(fitem.t2, undefined, fitem);
            fitem.ep.x = pos[0];
            fitem.ep.y = pos[1];
            ...
            var _pos = getCurPoint(fitem.t2, this.funcLst[len].ep, fitem);
        ...
        getAllTime() {
          ...
          for (var key in this.funcLst) {
            var v = this.funcLst[key];
            time = time + v.time;
          }
        ...
        getPosAndSlopeByTime(t) {
          ...
          var pos0, ftype, line, pos1;
          var res = getPostionByTime(t, this.funcLst, this.rotationAngle, this.startPos, this.curIndex);
          pos0 = res[0];
          ftype = res[1];
          line = res[2];
        ...
        getPosAndAngleByTime(t) {
          this.curActionSeconds = t;
          var pos, k, bcal;
          var res = this.getPosAndSlopeByTime(t); // 保存该路线的开始方向
        ...
        isCurveEnd() {
          var it = this.funcLst[this.funcLst.length - 1];
        ...
        setActionReserve(bres) {
          ...
          if (this.isReserve) {
            ...
            for (var key in this.funcLst) {
              ...
              var ep = i > 0 ? this.funcLst[i - 1].ep : undefined;
              var pos = getCurPoint(n.t2, ep, n);
              n.ep.x = pos[0];
              n.ep.y = pos[1];
              i++;
            }
...
//# sourceMappingURL=8c2ca5f2ec02e429367a42f9cb936ef07f86cd4b.js.map