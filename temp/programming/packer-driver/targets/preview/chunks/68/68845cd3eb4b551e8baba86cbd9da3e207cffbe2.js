      ...
      _export("Yxx_Footer", Yxx_Footer = (_dec = ccclass('Yxx_Footer'), _dec(_class = class Yxx_Footer extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        initChip() {
          ...
          (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            ...
            var node = this.createChip(chip, new Vec3(startX + index * offset, 20));
        ...
        createChip(chip, position) {
          var node = instantiate(this.propertyNode.props_bet_chip);
          node.setPosition(position);
          node.name = this.chipNodeNamePrefix + chip.value;
          this.bindChipEvent(node);
          var chips = node.getChildByName("spr_chips");
          var checkMark = node.getChildByName("Checkmark");
          var sf = (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
            error: Error()
          ...
          var chipLabel = chips.getChildByName("Label_chips").getComponent(Label);
          ...
          var checkMarkChipLabel = checkMark.getChildByName("Label_chips").getComponent(Label);
        ...
        getLockBet(gold) {
          var isLockBet = gold < (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
        ...
        chipShowHandle() {
          ...
          var chipNodes = this.propertyNode.props_bet_chip_group.children.filter(v => v.name.startsWith(this.chipNodeNamePrefix));
          var myGold = this.props.myHead.gold; // 是否锁定下注

          var isLockBet = myGold < (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
          }), config) : config).gameOption.unlockBetMinGold;
          ...
          for (var i = chipNodes.length - 1; i >= 0; i--) {
            var node = chipNodes[i];
            var _value = this.getChipConfig(node.name).value;
            var isChecked = false; // 如果当前金币大于等于选择的筹码，就去找到选择的筹码，让其选中

            if (myGold >= this.props.selectChip) {
              if (_value === this.props.selectChip) {
                isChecked = true;
              } // 如果当前金币小于之前选择的筹码，就去找最大的可选择的筹码

            } else if (myGold >= _value) {
              isChecked = true;
            }
        ...
        updateChipStatus(node, disable) {
          ...
          chips.active = true;
          var chipLabel = chips.getChildByName("Label_chips").getComponent(Label);
          var fileUrl = disable ? (_crd && FontPathDefine === void 0 ? (_reportPossibleCrUseOfFontPathDefine({
            error: Error()
        ...
        playAddGlodAnimation(winloss) {
          if (winloss <= 0) return;
          var startPosition = new Vec3(this.propertyNode.props_gold_up.position.x, this.propertyNode.props_gold_up.position.y);
...
//# sourceMappingURL=68845cd3eb4b551e8baba86cbd9da3e207cffbe2.js.map