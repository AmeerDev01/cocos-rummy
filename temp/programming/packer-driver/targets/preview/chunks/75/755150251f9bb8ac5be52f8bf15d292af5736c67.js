    ...
    execute: function () {
      ...
      _export("LockFish", LockFish = class LockFish {
        ...
        init(info) {
          ...
          var frame_name = info.lkey + '_1';
          var test_size = (_crd && getSpriteAtlasBySpriteFrame === void 0 ? (_reportPossibleCrUseOfgetSpriteAtlasBySpriteFrame({
            error: Error()
          ...
          var width = this.last_spr.getComponent(UITransform).width;
          var height = this.last_spr.getComponent(UITransform).height;
          this.last_spr_r = 0;
        ...
        lock() {
          ...
          if (this.lock_fish_lst.length > 0) {
            ...
            var cur_fsh_obj = this.lock_fish_lst[this.cur_lock_index];
        ...
        unlock() {
          ...
          for (var k = 0; k < this.spr_lock_line.length; k++) {
            var v = this.spr_lock_line[k];
            if (!v) continue;
            v.active = false;
          }
        }
        ...
        selectLockFish() {
          if (this.is_force_lock && this.is_local) {
            var fsh_obj = this.fishManager.getFishObjById(this.cur_lock_obj_id);
          ...
          if (this.is_local === false) {
            var _fsh_obj = this.fishManager.getFishObjById(this.cur_lock_obj_id);

            if (this.isFishCanLock(_fsh_obj)) {
              this.is_lock_success = true;
              this.is_lock_change = true;
              return _fsh_obj;
            }
          ...
          for (var k = 0; k < this.lock_fish_lst.length; k++) {
            var v = this.lock_fish_lst[k];
            if (!v) continue;
        ...
        isFishCanLock(fsh_obj) {
          if (fsh_obj) {
            var fpos = fsh_obj.getCurrentPos();
            var battery_obj = this.batteryManager.getBatteryByPlayerId(this.player_id);
            var bpos = battery_obj.getNoOffsetBulletPos(); // let rect = new Rect(0, 0, this.commonConfig.bonding_size.width, this.commonConfig.bonding_size.height)

            var dis = sqrt(pow(fpos.x - bpos.x, 2) + pow(fpos.y - bpos.y, 2));
        ...
        isPosValid(side, spos, epos) {
          var angle = this.getRotationAngle(spos, epos, side);
          var angle_valide = false;
        ...
        update(dt) {
          if (this.block === true) {
            var fsh_obj = this.selectLockFish();
            ...
            if (fsh_obj && this.is_lock_success === true) {
              ...
              var end_pos = fsh_obj.getCurrentPos().clone();
        ...
        resetLockLineByPos(pos1, pos2, battery_obj) {
          var angle = this.getRotationAngle(pos1, pos2, this.side);
          ...
          var distance = sqrt(pow(this.spos1.x - pos2.x, 2) + pow(this.spos1.y - pos2.y, 2));
          distance = distance - (this.line_r + this.last_spr_r);
          var show_index = 0;
          var dist_add = 0;
          var lst_len = this.spr_lock_line.length;
          ...
          for (var i = 1; i <= lst_len; i++) {
            ...
            var cal_dis = (i - 1) * (this.line_r * 2 + this.lock_spr_dis);
            var pos = this.getPosByDistanceAndAngle(cal_dis, angle); // this.spr_lock_line[i - 1].setWorldPosition(pos)
          ...
          var frame_name = this.lkey + "_1";

          while (dist_add < distance) {
            var obj = (_crd && createSpriteAtlasNode === void 0 ? (_reportPossibleCrUseOfcreateSpriteAtlasNode({
              error: Error()
            ...
            var _pos = this.getPosByDistanceAndAngle(dist_add, angle); // obj.setWorldPosition(pos)


            obj.setPosition((_crd && toNodeSpaceAR === void 0 ? (_reportPossibleCrUseOftoNodeSpaceAR({
              error: Error()
            }), toNodeSpaceAR) : toNodeSpaceAR)(this.batteryManager.getRootNode(), _pos));
          ...
          for (var _i = show_index; _i < this.spr_lock_line.length; _i++) {
            this.spr_lock_line[_i].active = false;
          }
        ...
        getPosByDistanceAndAngle(dis, angle) {
          ...
          var sx = dis * cos(ranti);
          var sy = dis * sin(ranti);
...
//# sourceMappingURL=755150251f9bb8ac5be52f8bf15d292af5736c67.js.map