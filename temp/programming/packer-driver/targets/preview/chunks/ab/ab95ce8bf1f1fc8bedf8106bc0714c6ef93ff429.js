    ...
    execute: function () {
      ...
      _export("SkillItemBossComing", SkillItemBossComing = class SkillItemBossComing {
        ...
        init(config, textureConfig) {
          ...
          var firstTexture = config.src_id_lst[0];
          var secondTexture = config.src_id_lst[1];

          if (firstTexture) {
            var src_cfg = textureConfig[firstTexture];
            ...
            var op = this.spr.addComponent(UIOpacity);
            this.spr.setPosition(0, 0);
            this.spr.setScale(new Vec3(1.5, 1.5));
            var scale = this.spr.getScale().multiplyScalar(this.skillPlayer.getScaleRatio());
          ...
          if (secondTexture) {
            var ani_config = textureConfig[secondTexture];
          ...
          for (var k = 0; k < config.icon_id_lst.length; k++) {
            var v = config.icon_id_lst[k];
            var ani_config1 = textureConfig[v];

            if (ani_config1) {
              var spriteFrame = String(ani_config1.pattern).format(1);
              var spr = (_crd && createSpriteAtlasNode === void 0 ? (_reportPossibleCrUseOfcreateSpriteAtlasNode({
                error: Error()
              }), createSpriteAtlasNode) : createSpriteAtlasNode)(this.sourceManage, this.uiRoot, ani_config1, spriteFrame);
              var spr_size = spr.getComponent(UITransform); // spr.setPosition(-568 - spr_size.width / 2 - 10, 0)
...
//# sourceMappingURL=ab95ce8bf1f1fc8bedf8106bc0714c6ef93ff429.js.map