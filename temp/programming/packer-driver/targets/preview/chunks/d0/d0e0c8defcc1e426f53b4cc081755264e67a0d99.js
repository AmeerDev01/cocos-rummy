      ...
      _export("Bandar_betArea", Bandar_betArea = (_dec = ccclass('Bandar_betArea'), _dec(_class = class Bandar_betArea extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
          ...
          if (key === "goldData") {
            ...
            if (value.cur && Object.keys(value.cur).length != 0) {
              for (var _key in value.cur) {
                var areaNode = this.getNodeByBetType(this.getNodeToBetType(Number(_key)));
                ...
                if (value.cur[_key] === 0) {
                  ...
                  (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
                    ...
                    error: Error()
                  }), formatNumber) : formatNumber)(value.cur[_key]);
                }
              }
            }
          }
          ...
          if (key === "memberBet") {
            ...
            if (Object.keys(value.cur).length != 0) {
              for (var _key2 in value.cur) {
                ...
                if (value.cur[_key2] === 0) {
                  (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
                    error: Error()
                  }), getNodeByNameDeep) : getNodeByNameDeep)("Label_bet_mine", _areaNode).getComponent(Label).string = "";
                } else {
                  (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
                    error: Error()
                  }), getNodeByNameDeep) : getNodeByNameDeep)("spr_bet_bg", _areaNode).active = true;
                  (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
                    error: Error()
                  }), getNodeByNameDeep) : getNodeByNameDeep)("Label_bet_mine", _areaNode).getComponent(Label).string = (_crd && formatNumber === void 0 ? (_reportPossibleCrUseOfformatNumber({
                    error: Error()
        ...
        myBet(betType) {
          ...
          if (this.props.gold < this.props.selectChip * 6) {
            // 提示用户余额不足
            var bet_remind = (_crd && footerViewModel === void 0 ? (_reportPossibleCrUseOffooterViewModel({
              error: Error()
        ...
        flyChip(betData, isFly) {
          ...
          var betArea = this.getNodeByBetType(betData.betType).getChildByName("Layout_bet_chips");
          var viewModel = this.createChip(betData.betAmount, betArea);
          var radom = Math.random() * 90 - 90; //旋转角度

          var chipNode = viewModel.comp.node;
          var endPosition = this.randomBetEndPosition(betArea);
        ...
        clearBetWinAreaAni(winArea) {
          winArea.forEach(item => {
            var winAreaNode = this.getNodeByBetType(item);
            (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
              error: Error()
        ...
        playWinArea(type) {
          var winAreaNode = this.getNodeByBetType(type);
          (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
            error: Error()
          }), getNodeByNameDeep) : getNodeByNameDeep)("animation_win1", winAreaNode).active = true;
          window.setTimeout(() => {
        ...
        randomBetEndPosition(betArea) {
          ...
          var width = uiTransform.width * 0.8;
          var height = uiTransform.height * 0.6;
        ...
        flyWinChip(winBetData, index, isFly) {
          if (isFly === void 0) {
            isFly = true;
          }

          var betInfos = this.betAreaInfo.get(winBetData.betType);
          ...
          isFly = this.isShowGame;
          var odds = this.props.cardRate[index] - 1;
          var betArea = this.getNodeByBetType(winBetData.betType).getChildByName("Layout_bet_chips"); // console.log("this.props.cardRate",this.props.cardRate);

          var copyBetInfo = this.copyBetInfo(betInfos, betArea, odds);
          var chips = [];
          copyBetInfo.forEach(v => chips = chips.concat(v.chips));
          var uiTransform = this.node.getComponent(UITransform); // log("healerFlyToBetArea === ", chips.length);

          chips.forEach(v => {
            var startPosition = new Vec3((_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
              error: Error()
            }), config) : config).gameOption.dealerPosition.x, (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
              error: Error()
            }), config) : config).gameOption.dealerPosition.y);
        ...
        cancelBet(cancelBetData) {
          ...
          if (cancelBetData) {
            var betInfos = this.betAreaInfo.get(cancelBetData.betType); // console.log("betInfos",betInfos);
            ...
            var betArea = this.getNodeByBetType(cancelBetData.betType).getChildByName("Layout_bet_chips"); // this.updateBetAreaGold(cancelBetData, betArea.parent);
...
//# sourceMappingURL=d0e0c8defcc1e426f53b4cc081755264e67a0d99.js.map