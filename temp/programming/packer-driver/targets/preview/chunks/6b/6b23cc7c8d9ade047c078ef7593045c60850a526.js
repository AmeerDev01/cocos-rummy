  ...
  return {
    ...
    execute: function () {
      ...
      _export("ObjectEffectWorld", ObjectEffectWorld = class ObjectEffectWorld {
        ...
        update() {
          var shotLst = {};
          var bullets = this.bulletManager.getBulletBondingInfoLst();

          for (var k in bullets) {
            var v = bullets[k];
            if (v === null) continue;
            var battery = this.batteryManager.getBatteryByPlayerId(v.player_id);

            if (battery) {
              var bullet = v.bullet;
              var lockFishID = battery.getLockFishId();
              var lockFish = this.fishManager.getFishObjById(lockFishID); // console.log(`lockFishID = ${lockFishID}, bulletLockFishID = ${bullet.getCurLockFishId()}`);
              ...
              if (lockFish && bullet.getCurLockFishId() === lockFishID) {
                ...
                //没有锁定
                var fish = this.fishManager.shotFish(v.bbx);
...
//# sourceMappingURL=6b23cc7c8d9ade047c078ef7593045c60850a526.js.map