    ...
    execute: function () {
      ...
      _export("DragonV2_Roller", DragonV2_Roller = (_dec = ccclass('DragonV2_Roller'), _dec(_class = class DragonV2_Roller extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        joinIcon() {
          ...
          var position = new Vec3(0, this.intervalHeight + icon.getHeight() / 2);
        ...
        thePresenceOrAbsenceOfARingOfFire(key, callbak) {
          // 检测是否火圈是否存在
          var children = this.propertyNode.props_pyrosphere.children;

          if (children.length > 0) {
            var arr = children.filter(item => item.key === key[1]);
        ...
        joinToStartMoving(key) {
          var node = instantiate((_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
            error: Error()
        ...
        renderTheRingOfFire(callBack) {
          ...
          if (children.length > 0) {
            children.forEach(item => {
              ...
              var ty = item.key < 0 ? -300 : item.key * (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
                error: Error()
              }), config) : config).normalRollOption.singleRollerHeight + (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
                error: Error()
        ...
        nodeMOve(item, targetY, isDestroy) {
          if (isDestroy === void 0) {
            isDestroy = false;
          }
        ...
        setBeetleAmount(icon) {
          ...
          if (lockIconData) {
            var amount = lockIconData.amount >= 10000 ? lockIconData.amount : lockIconData.amount;
        ...
        stopRoll(stopIconIndex, rollOverCallback, isSpeedBorder) {
          if (isSpeedBorder === void 0) {
            isSpeedBorder = false;
          }
        ...
        mergeIcon(parentNode) {
          ...
          if (iconCount === this.showIconCount) {
            ...
            for (var _i = 0; _i < this.showIconCount - 1; _i++) {
              if (this.iconArr[_i].getId() === this.iconArr[_i + 1].getId() && this.isWinIcon(_i) && this.isWinIcon(_i + 1)) {
                pos = this.iconArr[_i].getNode().worldPosition.clone().add(new Vec3(0, this.iconArr[_i].getHeight() / 2 + this.intervalHeight));
              }
            }
          } // if (pos) {
        ...
        playBaoHeAnimation(done) {
          var count = 0;
          var beetleArr = this.iconArr.filter((v, index) => v.getId() === (_crd && IconId === void 0 ? (_reportPossibleCrUseOfIconId({
            error: Error()
        ...
        rollOverHandle() {
          ...
          var time = this.isSubGame2() ? 1.5 : 0.05; // const time = 0.05;
        ...
        addLockIcon() {
          ...
          lockValues.forEach(v => {
            var DragonV2Icon = this.lockIcons.find(iconv => iconv.getIconIndex() === v.index);
            ...
            if (!DragonV2Icon) {
              ...
              var pot = DragonV2Icon.getPosition();
        ...
        buildSingleRollerIcon(index, pos, icon) {
          var viewModel = new (_crd && DragonV2SingleIconRollerViewModel === void 0 ? (_reportPossibleCrUseOfDragonV2SingleIconRollerViewModel({
            error: Error()
...
//# sourceMappingURL=3d0dc900fa7ec180a970a39e9e57e974010f4c86.js.map