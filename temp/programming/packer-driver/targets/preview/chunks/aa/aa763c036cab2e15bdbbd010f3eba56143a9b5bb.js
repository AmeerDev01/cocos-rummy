      ...
      _export("EgyptV2_RollerPanel", EgyptV2_RollerPanel = (_dec = ccclass('EgyptV2_RollerPanel'), _dec(_class = class EgyptV2_RollerPanel extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        initRoller() {
          for (var i = 0; i < (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            ...
            var roller = new (_crd && EgyptV2RollerViewModel === void 0 ? (_reportPossibleCrUseOfEgyptV2RollerViewModel({
              error: Error()
        ...
        updateWinloss(rollerLaunchResult) {
          var si = rollerLaunchResult.dl.si[0]; // 本局输赢
        ...
        playBeeteFlyAmount(done) {
          ...
          var count = 0;
          nodes.forEach(v => {
            var startPos = this.node.parent.getComponent(UITransform).convertToNodeSpaceAR(v.getAmountNode().getWorldPosition());
            var cloneNode = instantiate(v.getAmountNode());
        ...
        playBeeteFlyAmountToSunGod(done) {
          ...
          var sunGodIcons = this.getWinSunGodIconNode();
        ...
        eachSunGodIcon(index, sunGodIcons, done) {
          var sunGodIcon = sunGodIcons[index];
          var nodes = [];
          this.rollerViewModelArr.forEach(v => {
            nodes = nodes.concat(v.comp.getBeetleNode(true));
          }); // 按照行的顺序排序，从上往下
          ...
          var endPos = this.node.parent.getComponent(UITransform).convertToNodeSpaceAR(sunGodIcon.getNode().worldPosition);
        ...
        playAmountToSunGodCourse(nodes, sunGoldIcon, index, beetleFlyTime, endPos, done) {
          var egyptV2Icon = nodes[index];
          var startPos = this.node.parent.getComponent(UITransform).convertToNodeSpaceAR(egyptV2Icon.getAmountNode().getWorldPosition());
          var cloneNode = instantiate(egyptV2Icon.getAmountNode());
          this.node.parent.addChild(cloneNode);
...
//# sourceMappingURL=aa763c036cab2e15bdbbd010f3eba56143a9b5bb.js.map