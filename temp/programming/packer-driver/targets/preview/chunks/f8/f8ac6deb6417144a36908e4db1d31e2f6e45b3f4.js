      ...
      _export("Starlight_HelpPanel", Starlight_HelpPanel = (_dec = ccclass('Starlight_HelpPanel'), _dec(_class = class Starlight_HelpPanel extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        renderingRules(num) {
          ...
          var _loop = function _loop(i) {
            _this.propertyNode.props_spr_page1.children.forEach(item => {
              if (item.name === i) {
                item.getChildByName('Label_rate_num1').getChildByName('label_num').getComponent(Label).string = (_this.basicMagnification[i][0] * num).formatAmountWithLetter();
                item.getChildByName('Label_rate_num2').getChildByName('label_num').getComponent(Label).string = (_this.basicMagnification[i][1] * num).formatAmountWithLetter();
                item.getChildByName('Label_rate_num3').getChildByName('label_num').getComponent(Label).string = (_this.basicMagnification[i][2] * num).formatAmountWithLetter();
              }
            });
          };

          for (var i in this.basicMagnification) {
            _loop(i);
          }
        }
...
//# sourceMappingURL=f8ac6deb6417144a36908e4db1d31e2f6e45b3f4.js.map