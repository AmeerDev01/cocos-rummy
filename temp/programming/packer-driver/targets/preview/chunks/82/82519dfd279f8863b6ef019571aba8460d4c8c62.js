      ...
      _export("Common_Marquee", Common_Marquee = (_dec = ccclass('Common_Marquee'), _dec(_class = class Common_Marquee extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        run() {
          if (this.marueeMap.length) {
            var fristItem = this.marueeMap[0];
            ...
            var itemNode = instantiate(this.propertyNode.props_template_label_ticker); // itemNode.getComponent(Label).string = decodeURI(fristItem.content)
            // const cocosString = decodeURI(fristItem.content).replace(/<span\s+style=['"]color\s*:\s*#([^'"]+)['"]>([^<]+)<\/span>/gi, (match, color, content) => {
            // 	// 返回转换后的格式
            // 	return `<color=#${color}>${content}</color>`;
            // })

            var cocosString = this.convertHtmlToCustomFormat(decodeURI(fristItem.content));
        ...
        convertHtmlToCustomFormat(inputString) {
          var regex = /<span\s+style=['"]color\s*:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\);['"]>([^<]+)<\/span>/gi;
          var convertedString = inputString.replace(regex, (match, r, g, b, content) => {
            var hexColor = this.rgbToHex(parseInt(r), parseInt(g), parseInt(b));
        ...
        rgbToHex(r, g, b) {
          var _r = Math.max(0, Math.min(255, r));

          var _g = Math.max(0, Math.min(255, g));

          var _b = Math.max(0, Math.min(255, b));
...
//# sourceMappingURL=82519dfd279f8863b6ef019571aba8460d4c8c62.js.map