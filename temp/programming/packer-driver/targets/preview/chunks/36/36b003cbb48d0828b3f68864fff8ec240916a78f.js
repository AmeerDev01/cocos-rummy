    ...
    execute: function () {
      ...
      _export("phoenix_rollerIcon", phoenix_rollerIcon = (_dec = ccclass('phoenix_rollerIcon'), _dec(_class = class phoenix_rollerIcon extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        useProps(key, value) {
          ...
          if (key === "iconEffect") {
            ...
            if (this.props.iconData.uiData.id === 13) {
              _iconNodeSprite.getComponent(sp.Skeleton).skeletonData = (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
                error: Error()
              }), sourceManageSeletor) : sourceManageSeletor)().getFile(this.props.iconData.uiData.fileName).source;
            ...
            if (isActive) {
              ...
              var t = tween(uiOpacity).repeatForever(tween().to(0.4, {
                opacity: 255
              }).to(0.4, {
                opacity: 0
              }));
            ...
            var skeletal = _iconNodeSprite.getComponent(sp.Skeleton);
          ...
          if (key === "iconYuanBaoEffect") {
            ...
            console.log("yuanbao", _isActive);

            var _skeletal = _iconNodeSprite2.getComponent(sp.Skeleton);

            _skeletal.updateRenderer();

            _skeletal.skeletonData = (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
              error: Error()
            ...
            _skeletal.animation = this.props.iconData.uiData.animationArr[_isActive ? 1 : 0];
            ...
            if (_isActive) {
              this.showYinBi();
              tween(_iconNodeSprite2).delay(1).call(() => {
                /**展示元宝中奖框 */
                if (!this.node) {
                  return;
                }
              ...
              new (_crd && SkeletonAnimationPlayer === void 0 ? (_reportPossibleCrUseOfSkeletonAnimationPlayer({
                error: Error()
              }), SkeletonAnimationPlayer) : SkeletonAnimationPlayer)(_skeletal, "animation", false).stopAnimation();
            }
          }
          ...
          if (key === "iconFreeGameEffect") {
            ...
            var _iconNodeSprite3 = this.propertyNode.props_iconWrap.getChildByName("iconNode_sprite");

            var _isActive2 = value.cur === (_crd && IconEffectType === void 0 ? (_reportPossibleCrUseOfIconEffectType({
              error: Error()
            }), IconEffectType) : IconEffectType).FREEEGAMEZ && !this.props.iconData.LargeIcon;

            var _skeletal2 = _iconNodeSprite3.getComponent(sp.Skeleton);

            _skeletal2.updateRenderer();

            _skeletal2.skeletonData = (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
              error: Error()
            }), sourceManageSeletor) : sourceManageSeletor)().getFile((_crd && SkeletalPathDefine === void 0 ? (_reportPossibleCrUseOfSkeletalPathDefine({
              error: Error()
            }), SkeletalPathDefine) : SkeletalPathDefine).TUBIAO_SKELETON).source;
            _skeletal2.animation = this.props.iconData.uiData.animationArr[_isActive2 ? 1 : 0];

            var {
              x: _x3,
              y: _y3,
              z: _z3
            } = _iconNodeSprite3.getPosition();

            _iconNodeSprite3.setPosition(new Vec3(_x3 + this.props.iconData.uiData.distance[0], _y3 + this.props.iconData.uiData.distance[1], _x3));

            if (!_isActive2) {
              new (_crd && SkeletonAnimationPlayer === void 0 ? (_reportPossibleCrUseOfSkeletonAnimationPlayer({
                error: Error()
              }), SkeletonAnimationPlayer) : SkeletonAnimationPlayer)(_skeletal2, "animation", false).stopAnimation();
            }
          }
        }
        ...
        playallPurpose(callBack) {
          ...
          var startWorldPos = this.propertyNode.props_Particle2D.worldPosition; // console.log(footerViewModel.comp.getPropertyNode().props_btn_down_putar as Node);

          var targetWorldPos = (_crd && footerViewModel === void 0 ? (_reportPossibleCrUseOffooterViewModel({
            error: Error()
          }), footerViewModel) : footerViewModel).comp.getPropertyNode().props_btn_down_putar.worldPosition; // 创建节点的实例

          var node = instantiate(this.propertyNode.props_Particle2D); // 将节点添加到目标节点所在的父节点
          ...
          var uiTransform = (_crd && footerViewModel === void 0 ? (_reportPossibleCrUseOffooterViewModel({
            error: Error()
          ...
          var targetLocalPos = uiTransform.convertToNodeSpaceAR(targetWorldPos); // 设置节点的初始位置
        ...
        showYinBi() {
          ...
          var startWorldPos = this.propertyNode.props_yinbi.worldPosition; // console.log(footerViewModel.comp.getPropertyNode().props_btn_down_putar as Node);

          var targetWorldPos = (_crd && footerViewModel === void 0 ? (_reportPossibleCrUseOffooterViewModel({
            error: Error()
          }), footerViewModel) : footerViewModel).comp.getPropertyNode().props_word_down_winNum.worldPosition; // 创建节点的实例

          var node = instantiate(this.propertyNode.props_yinbi); // 将节点添加到目标节点所在的父节点
...
//# sourceMappingURL=36b003cbb48d0828b3f68864fff8ec240916a78f.js.map