  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", BaseViewLayer = ccclass(_class = class BaseViewLayer extends Component {
        ...
        static bindNode(obj) {
          const data = obj._bindNode;

          for (const key in data) {
            const item = data[key];
            const res = item.path == "/" ? obj.node : find(item.path, obj.node);

            if (!res) {// Log.e("res>>>>>>", item.path, obj.node)
            }

            if (item.type) {
              obj[key] = res.getComponent(item.type) || res.addComponent(item.type);
            } else {
              obj[key] = res;
            }
          }
...
//# sourceMappingURL=f96cc45bbbe77168a51c1829c80ca9eb574e06d3.js.map