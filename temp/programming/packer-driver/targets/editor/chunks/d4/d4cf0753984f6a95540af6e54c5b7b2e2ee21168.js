    ...
    execute: function () {
      ...
      _export("Qiuqiu_MainBoard", Qiuqiu_MainBoard = (_dec = ccclass('Qiuqiu_MainBoard'), _dec(_class = class Qiuqiu_MainBoard extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        flyClip() {
          ...
          const chips = (_crd && splitChip === void 0 ? (_reportPossibleCrUseOfsplitChip({
            error: Error()
        ...
        createChip(startPosIsWorlds, startPos, endPos, value, parentNode, speed, done = null) {
          let chip = new (_crd && ChipViewModel === void 0 ? (_reportPossibleCrUseOfChipViewModel({
            error: Error()
        ...
        pileChip(startPos) {
          const wins = this.props.balanceInfos; // 通过递归，第一堆往用户头像飞完之后，接到飞第二堆

          const flyToPlayer = arrIndex => {
            ...
            this.scheduleOnce(() => {
              this.node.isValid && this.createChip(true, dealerPos, endPos, chips[i], startNode, this.flyChipSpeed, () => {
                if (i === length - 1) {
                  // 停顿一下再放用户头像飞
                  // this.scheduleOnce(() => {
                  this.node.isValid && done && done(); // }, 0.5)
                }
              });
              endPos.y += 5;
            }, 0.05 * (i + 1));
        ...
        chipFlyToPlayer(pildChipNode, flyToSeatIndex, done) {
          const endPos = this.events.getSeatFlyClipStartPos(flyToSeatIndex); // 飞向自己时播放收金币的音效
          ...
          for (let i = 0; i < length; i++) {
            const v = children[i];
            tween(v).to(this.flyChipSpeed * (i + 1), {
              worldPosition: endPos
            }).removeSelf().call(() => {
              if (i === length - 1) {
                done && done();
...
//# sourceMappingURL=d4cf0753984f6a95540af6e54c5b7b2e2ee21168.js.map