    ...
    execute: function () {
      ...
      _export("Battery", Battery = class Battery extends Component {
        ...
        rotationBatteryByPos(pos) {
          ...
          let rotate_angle = this.getBatteryRotationAngle(this.cur_angle); // console.log(pos, this.local_pos, rotate_angle);
        ...
        getRotationAngle(pos1, pos2, side) {
          ...
          if (pos1.x === pos2.x) {
            ...
            let k = (pos1.y - pos2.y) / (pos1.x - pos2.x); // angle =  Math.atan2(pos1.y - pos2.y, pos1.x - pos2.x) * 180 / Math.PI
        ...
        updateEgrAction() {
          ...
          if (this.user_info.isPowerBattery) {
            if (sprEgr.active === false) {
              ...
              const an = (_crd && getAnimation === void 0 ? (_reportPossibleCrUseOfgetAnimation({
                error: Error()
        ...
        getSendBulletPos() {
          let ranti = this.cur_angle / (_crd && pia === void 0 ? (_reportPossibleCrUseOfpia({
            error: Error()
          }), pia) : pia) * (_crd && pi === void 0 ? (_reportPossibleCrUseOfpi({
            error: Error()
        ...
        update(dt) {
          ...
          if (this.isLockFish()) {
            ...
            let rotate_angle = this.getBatteryRotationAngle(this.cur_angle); // if (this.side === 1) {
        ...
        runBatteryFireAni() {
          ...
          if (this.spr_obj) {
            const an = this.spr_obj.getComponent(Animation); // an.stop();
        ...
        setScoreVal(val) {
          if (this.dataManager.getRoomType() === GAME_TYPE_FRIENDROOM) {
            let show_val = (_crd && llminus === void 0 ? (_reportPossibleCrUseOfllminus({
              error: Error()
            ...
            if ((_crd && llcompare === void 0 ? (_reportPossibleCrUseOfllcompare({
              error: Error()
            }), llcompare) : llcompare)(show_val, 0) < 0) {
              show_val = 0;
            }

            this.ui_lst.getChildByName('fnt_score').getComponent(Label).string = `${show_val}`;
            let room_info = this.dataManager.getRoomInfo(this.user_info.room_id); // if (this.user_info.is_local === false) {
...
//# sourceMappingURL=f4c5d8cbdaf0ad3ac3669e82062eb6187506d4aa.js.map