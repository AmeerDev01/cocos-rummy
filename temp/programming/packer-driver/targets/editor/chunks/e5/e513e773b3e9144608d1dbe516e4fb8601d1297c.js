    ...
    execute: function () {
      ...
      _export("Hall_MainPanel", Hall_MainPanel = (_dec = ccclass('Hall_MainPanel'), _dec(_class = class Hall_MainPanel extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        callback(event, customEventData) {
          ...
          let gameType = (_crd && SubGameType === void 0 ? (_reportPossibleCrUseOfSubGameType({
            error: Error()
        ...
        bindUI() {
          this.useState(() => {
            // 实例化图标对象，先清空
            const scrollViewContent = this.propertyNode.props_ScrollView_game_icon.getComponent(ScrollView).content; // scrollViewContent.removeAllChildren()
            // const gateViewModelList: BaseViewModel<Hall_SubGameGate, GateState, GateProps, GateEvent>[] = []

            const ratio = (_crd && setByScreenScale === void 0 ? (_reportPossibleCrUseOfsetByScreenScale({
              error: Error()
            ...
            this.state.currGateQueue.queue.forEach(gamesIds => {
              ...
              if (!isInit) {
                const target = this.gateViewModelList.find(item => (_crd && equal === void 0 ? (_reportPossibleCrUseOfequal({
                  error: Error()
                }), equal) : equal)(item.comp.props.gamesIds, gamesIds));
                target && (target.viewNode.active = true);
              } else {
                const gateModelView = new (_crd && SubGameGateViewModel === void 0 ? (_reportPossibleCrUseOfSubGameGateViewModel({
                  error: Error()
        ...
        getGetSubGameGateQueue(tabName, typeName) {
          // 获取当前要显示的图标数据
          const subGameQueue = (_crd && subGameGateQueue === void 0 ? (_reportPossibleCrUseOfsubGameGateQueue({
            error: Error()
...
//# sourceMappingURL=e513e773b3e9144608d1dbe516e4fb8601d1297c.js.map