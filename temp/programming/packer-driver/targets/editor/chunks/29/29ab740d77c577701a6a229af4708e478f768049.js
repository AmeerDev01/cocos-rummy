    ...
    execute: function () {
      ...
      _export("Yxx_GameBoard", Yxx_GameBoard = (_dec = ccclass('Yxx_GameBoard'), _dec(_class = class Yxx_GameBoard extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        bindEvent1() {
          ...
          this.getSkeleton(this.propertyNode.props_open_gai).setCompleteListener(index => {
            ...
            this.scheduleOnceEnhance(() => {
              ...
              const balanceTime = 3; // 结算面板打开之后，给一定延时清除数据
        ...
        addDice() {
          ...
          const positions = this.randomDicePosition(); // 同时添加开盖盘子和关闭盖子盘子上面的骰子
        ...
        randomDicePosition() {
          const index = Math.floor(Math.random() * (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
          }), config) : config).dicePosition.length);
          const arr = [...(_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
            error: Error()
...
//# sourceMappingURL=29ab740d77c577701a6a229af4708e478f768049.js.map