    ...
    execute: function () {
      ...
      _export("DragonTiger_MainPanel", DragonTiger_MainPanel = (_dec = ccclass("DragonTiger_MainPanel"), _dec2 = property(Node), _dec(_class = (_class2 = class DragonTiger_MainPanel extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
            ...
            if (Object.keys(value.cur).length != 0) {
              for (let key in value.cur) {
                ...
                if (value.cur[key] === 0) {
                  ...
                  (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
                    error: Error()
                  }), getNodeByNameDeep) : getNodeByNameDeep)("Label_bet_mine", areaNode).getComponent(Label).string = value.cur[key].formatAmountWithCommas();
                }
              }
            }
          }
        ...
        channelToggleCallback(event, customEventData) {
          ...
          const toggleArr = event.target["parent"].children;

          for (let i = 0; i < toggleArr.length; i++) {
            if (toggleArr[i].getComponent(Toggle).isChecked) {
              let num = (_crd && config === void 0 ? (_reportPossibleCrUseOfconfig({
                error: Error()
        ...
        */
        ...
        randomBetEndPosition(betType) {
          let y = Math.floor(Math.random() * -150 + 50); //目标位置Y
        ...
        changeHistoryMin(list) {
          if (this.node && this.node.isValid) {
            for (let i = 0; i < list.length; i++) {
              ...
              let gameY = gameWinIcon.position.y;
              let gameX = gameWinIcon.position.x + gameWinIcon.getComponent(UITransform).contentSize.width * i;
              ...
              if (list[list.length - 1] && list.length > 0) {
                let newIconX = gameX + gameWinIcon.getComponent(UITransform).contentSize.width * 1 / 2;
                let newIconY = gameY + gameWinIcon.getComponent(UITransform).contentSize.height / 2;
        ...
        closePopup() {
          ...
          let time = this.initSeconds * 1000;
          window.setTimeout(() => {
            if (this.node && this.node.isValid) {
              this.removeChip();
        ...
        repeatBet() {
          ...
          let toggleArr = this.propertyNode.props_ToggleGroup.children;
          this.clearRepeatBet();
          ...
          if (isBet) {
            ...
            if (this.props.gold >= total) {
              ...
              for (let i = 0; i < lastBetArr.length; i++) {
                if (lastBetArr[i].typeTotal <= 0) {
                  continue;
                }

                ;
                lastBetArr[i].betId = (_crd && getUUID === void 0 ? (_reportPossibleCrUseOfgetUUID({
                  error: Error()
                }), getUUID) : getUUID)();
                let chipNode = instantiate((_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
                  error: Error()
                }), sourceManageSeletor) : sourceManageSeletor)().getFile((_crd && PrefabPathDefine === void 0 ? (_reportPossibleCrUseOfPrefabPathDefine({
                  error: Error()
                }), PrefabPathDefine) : PrefabPathDefine).MAIN_CHIP).source);
                let positionX = this.propertyNode.props_spr_user_head.position.x;
                let positionY = this.propertyNode.props_spr_user_head.position.y; // 设置下注金币初始位置

                chipNode.setPosition(positionX, positionY);
                const chips = [];
                (_crd && usersViewModel === void 0 ? (_reportPossibleCrUseOfusersViewModel({
                  error: Error()
                }), usersViewModel) : usersViewModel).comp.splitChip(lastBetArr[i].typeTotal, chips);
                chips.forEach(chip => {
                  const sendBet = {
                    roomId: (_crd && gameCacheData === void 0 ? (_reportPossibleCrUseOfgameCacheData({
                      error: Error()
                  ...
                  const betData = (_crd && initBetData === void 0 ? (_reportPossibleCrUseOfinitBetData({
                    error: Error()
                  }), initBetData) : initBetData)(this.props.myInfo.index, this.props.myInfo.memberId, lastBetArr[i].type, chip);
        ...
        stopBet() {
          let toggleArr = this.propertyNode.props_ToggleGroup.children; // 5-0倒计时结束节点 隐藏
        ...
        sendCard() {
          ...
          if (this.node && this.node.isValid) {
            ...
            ;
            let sendCardNode = (_crd && getNodeByNameDeep === void 0 ? (_reportPossibleCrUseOfgetNodeByNameDeep({
              error: Error()
        ...
        shuffleCard() {
          ...
          if (this.node && this.node.isValid) {
            if (this.props && this.props.pokerInfo) {
              let dragonTigerLeftCard = { ...this.props.pokerInfo.pokerLeftNum
              };
              let dragonTigerRightCard = { ...this.props.pokerInfo.pokerRightNum
              };
              (_crd && bundleDragonTiger === void 0 ? (_reportPossibleCrUseOfbundleDragonTiger({
                error: Error()
        ...
        judgeCoin(gold) {
          ...
          this.propertyNode.props_ToggleGroup.children.forEach(item => {
            let amountNum = item.getChildByName("spr_chips").getChildByName("Label_chips").getComponent(Label).string;
            ...
            if (gold < Number(amountNum)) {
              ...
              let amountNum = item.getChildByName("spr_chips").getChildByName("Label_chips").getComponent(Label).string;
              let font = amountNum == "1000" ? (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
                error: Error()
              }), sourceManageSeletor) : sourceManageSeletor)().getFile((_crd && FontPathDefine === void 0 ? (_reportPossibleCrUseOfFontPathDefine({
                error: Error()
              }), FontPathDefine) : FontPathDefine).GREEN_BET).source : amountNum == "5000" ? (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
                error: Error()
              }), sourceManageSeletor) : sourceManageSeletor)().getFile((_crd && FontPathDefine === void 0 ? (_reportPossibleCrUseOfFontPathDefine({
                error: Error()
              }), FontPathDefine) : FontPathDefine).BLUE_BET).source : amountNum == "10K" ? (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
                error: Error()
              }), sourceManageSeletor) : sourceManageSeletor)().getFile((_crd && FontPathDefine === void 0 ? (_reportPossibleCrUseOfFontPathDefine({
                error: Error()
              }), FontPathDefine) : FontPathDefine).DARK_BLUE_BET).source : amountNum == "100K" ? (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
                error: Error()
              }), sourceManageSeletor) : sourceManageSeletor)().getFile((_crd && FontPathDefine === void 0 ? (_reportPossibleCrUseOfFontPathDefine({
                error: Error()
              }), FontPathDefine) : FontPathDefine).RED_BET).source : amountNum == "500K" ? (_crd && sourceManageSeletor === void 0 ? (_reportPossibleCrUseOfsourceManageSeletor({
                error: Error()
              }), sourceManageSeletor) : sourceManageSeletor)().getFile((_crd && FontPathDefine === void 0 ? (_reportPossibleCrUseOfFontPathDefine({
                error: Error()
        ...
        */
        ...
        cancelBet(cancelBetData) {
          if (cancelBetData) {
            ...
            const betArea = this.getNodeByBetType(cancelBetData.betType).getChildByName("Layout_bet"); // this.updateBetAreaGold(cancelBetData, betArea.parent);
...
//# sourceMappingURL=a4728fec653fb3573e1592243753ae33394d30bc.js.map