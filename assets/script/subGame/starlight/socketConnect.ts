...
export let starlightWebSocketDriver: WebSocketDriver<SKT_MAG_TYPE> = null
export default () => {
  ...
  return new Promise((resolve, reject) => {
    initConfig().then(() => {
      ...
      WebSocketStarter.Instance().initSocket().then(() => {
        starlightWebSocketDriver = new WebSocketDriver<SKT_MAG_TYPE>(gameId, gameHost)
        starlightWebSocketDriver.filterData = (data,source) => {
        ...
        //   return false
        // })
        resolve(starlightWebSocketDriver)
      })
    }).catch((e) => {
      reject(e)
    })
...
export const removeInstance = () => {
  starlightWebSocketDriver && starlightWebSocketDriver.logoutGame(SKT_MAG_TYPE.EXIT);
}