...
import socketConnect, { SKT_MAG_TYPE, gameLogin, removeInstance } from "./socketConnect";
...
export let bundleStarlight: AssetManager.Bundle = null
...
export let starlight_Audio: AudioMgr<SoundPathDefine>
let loaderviweModel: LoaderPanelViewModel;
export const sourceManageSeletor = (bundleName: string = 'starlight') => sourceManageMap.find(i => i.bundle.name === bundleName)
...
export const startUp = (rootNode: Node) => {
  ...
  assetManager.loadBundle(bundlePkgName, (err, bundle) => {
    bundleStarlight = bundle
    bundleStarlight.load(PrefabPathDefine.LOAING_PANEL, Prefab, (progress, total) => {
      ...
      loaderviweModel = new LoaderPanelViewModel().mountView(prefab).appendTo(rootNode).setProps({
        ...
        onLoadDone: (_sourceManageMap) => {
          ...
          starlight_Audio = new AudioMgr<SoundPathDefine>(sourceManageSeletor())
          setActiveAudio(starlight_Audio)
          ...
          socketConnect().then(() => {
            ...
            roleViewModel = new RoleViewModel().mountView(sourceManageSeletor().getFile(PrefabPathDefine.STARLIGHTROLE).source).appendTo(gameBoardViewModel.comp.getWrapNode()).connect()
