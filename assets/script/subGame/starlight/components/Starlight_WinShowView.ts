...
@ccclass('Starlight_WinShowView')
export class Starlight_WinShowView extends BaseComponent<IState, IProps, IEvent> {
			...
			if (value.cur.count >= 0) {
				...
				tween(node)
					...
					.call(() => {
						...
						this.propertyNode.props_spr_win_box && (new StepNumber(Number(str), this.total, (num) => {
							this.propertyNode && (this.propertyNode.props_spr_win_box.getChildByName('props_label_gold').getComponent(Label).string = (+num).formatAmountWithCommas())
						})).start()
					})
					// 添加一个简短的回弹动作
	...
	private calcMultiple(rate) { // 倍数相乘
		if (!this.propertyNode) return
		let str = this.propertyNode.props_spr_win_box.getChildByName('props_label_gold').getComponent(Label).string
		if (str) {
			str = str.replace(/,/g, "")
