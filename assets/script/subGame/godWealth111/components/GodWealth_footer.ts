...
import { godWealth_Audio, msgListener, NORMAL_MAG_TYPE, playMinCoin, playMaxCoin, playBtnClick } from '../index';
...
@ccclass('GodWealth_footer')
export class GodWealth_footer extends BaseComponent<IState, IProps, IEvent> {
	start() { }
	...
	protected useProps(key: keyof IProps, value: { pre: any, cur: any }) {
		if (key === "isRollEnd") {
			...
			if (this.props.currGameType !== GameType.MAIN || this.props.viewGameType !== GameType.MAIN) {
				this.setState({ disableBetSwitch: true })
			} else {
		...
		if (key === "subGameTimes" || key === "remainSubGameTimes") {
			console.log("remainSubGameTimes",this.props.remainSubGameTimes);
	...
	protected bindUI(): void {
		sktMsgListener.addOnce(SKT_MAG_TYPE.AUTH, "footer", () => {
			const target = this.getBetListByLevel()
			if (UseSetOption.Instance().option.gameSet.godWealth && UseSetOption.Instance().option.gameSet.godWealth.betTarget) {
				const t = target.find(i => i.positionId === UseSetOption.Instance().option.gameSet.godWealth.betTarget.positionId)
				this.setState({ baseBetAmountTarget: t || target[0] })
				rollerPanelViewModel.comp.getGrandNode().string = t.value * 15000 + '';
				rollerPanelViewModel.comp.getMajorNode().string = t.value * 1500 + '';
				rollerPanelViewModel.comp.getMiniNode().string = t.value * 750 + '';	
			} else {
				this.setState({ baseBetAmountTarget: target[0] })
				rollerPanelViewModel.comp.getGrandNode().string = target[0].value * 15000 + '';
				rollerPanelViewModel.comp.getMajorNode().string = target[0].value * 1500 + '';
				rollerPanelViewModel.comp.getMiniNode().string = target[0].value * 750 + '';
			}
		}, 2)
		...
		this.useState((key, value) => {
			...
			UseSetOption.Instance().setGameOption("godWealth", { betTarget: value.cur });
			...
			let majorNode = rollerPanelViewModel.comp.getMajorNode();
			let miniNode = rollerPanelViewModel.comp.getMiniNode();
			...
			this.taskScheduler.joinqQueue(new Task((done) => {
				this.stepNumber && this.stepNumber.stop()
				this.stepNumber = new StepNumber(startNumberMini, value.cur.value * 750, (num) => {
			...
			this.taskScheduler.joinqQueue(new Task((done) => {
				this.stepNumber && this.stepNumber.stop()
				this.stepNumber = new StepNumber(startNumberMajor, value.cur.value * 1500, (num) => {
			...
			if (value.cur.value < 20) {
				...
				this.taskScheduler.joinqQueue(new Task((done) => {
					this.stepNumber && this.stepNumber.stop()
					this.stepNumber = new StepNumber(startNumberGrand, value.cur.value *15000, (num) => {
