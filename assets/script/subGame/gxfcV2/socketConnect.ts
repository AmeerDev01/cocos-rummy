...
export let gxfcWebSocketDriver: WebSocketDriver<SKT_MAG_TYPE> = null
export default () => {
  ...
  return new Promise((resolve, reject) => {
    initConfig().then(() => {
      ...
      WebSocketStarter.Instance().initSocket().then(() => {
        gxfcWebSocketDriver = new WebSocketDriver<SKT_MAG_TYPE>(gameId, gameHost)
        gxfcWebSocketDriver.filterData = (data,source) => {
...
export const removeInstance = () => {
  gxfcWebSocketDriver && gxfcWebSocketDriver.logoutGame(SKT_MAG_TYPE.EXIT);
}