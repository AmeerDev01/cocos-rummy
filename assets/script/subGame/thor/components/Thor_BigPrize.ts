...
@ccclass('Thor_BigPrize')
export class Thor_BigPrize extends BaseComponent<IState, IProps, IEvent> {
	glodAmount = 0
	start() { }
	...
	protected useProps(key: keyof IProps, value: { pre: any, cur: any }) {
		if (key === "winning") {
			...
			if (value.cur) {
				...
				let nanzhuAnimation = new SkeletonAnimationPlayer(titleSkeleton, 'start', false, () => { // 动画开始
					// 显示数字
