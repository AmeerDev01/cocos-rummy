...
export class DragonV2Icon {
  ...
  constructor(iconConfig, isPoolObject: boolean = true) {
    ...
    this.faceSubgame2 = getNodeByNameDeep("face-subgame2", this.node);
  ...
  public playBaoHeAnimation(done) {
    ...
    if(this.currentStatus){
      this.faceAnimationNode.active = false;
      this.faceSubgame2.active = true;
      new SkeletonAnimationPlayer(this.faceSubgame2.getComponent(sp.Skeleton), this.iconConfig.animationArr[1], false, () => {
        this.faceSubgame2.active = false
      }).playAnimation()
    }else{
