...
export let egyptWebSocketDriver: WebSocketDriver<SKT_MAG_TYPE> = null
export default () => {
  ...
  return new Promise((resolve, reject) => {
    initConfig().then(() => {
      ...
      WebSocketStarter.Instance().initSocket().then(() => {
        egyptWebSocketDriver = new WebSocketDriver<SKT_MAG_TYPE>(gameId, gameHost)
        egyptWebSocketDriver.filterData = (data, source) => {
...
export const removeInstance = () => {
  egyptWebSocketDriver && egyptWebSocketDriver.logoutGame(SKT_MAG_TYPE.EXIT);
}