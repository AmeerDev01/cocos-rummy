...
export let starlightV2WebSocketDriver: WebSocketDriver<SKT_MAG_TYPE> = null
export default () => {
  ...
  return new Promise((resolve, reject) => {
    initConfig().then(() => {
      ...
      WebSocketStarter.Instance().initSocket().then(() => {
        starlightV2WebSocketDriver = new WebSocketDriver<SKT_MAG_TYPE>(gameId, gameHost)
        starlightV2WebSocketDriver.filterData = (data, source) => {
        ...
        resolve(starlightV2WebSocketDriver)
      })
    }).catch((e) => {
      reject(e)
    })
...
export const starlightGameLogin = () => {
  const msgObj = starlightV2WebSocketDriver.loginGame(SKT_MAG_TYPE.LOGIN)
...
export const removeInstance = () => {
  starlightV2WebSocketDriver && starlightV2WebSocketDriver.logoutGame(SKT_MAG_TYPE.EXIT);
}