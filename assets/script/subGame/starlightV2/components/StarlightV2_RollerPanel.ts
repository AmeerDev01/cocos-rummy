...
import StarlightV2RollerViewModel from '../viewModel/StarlightV2RollerViewModel';
...
import { starlightv2_Audio, mainViewModel, sourceManageSeletor } from '../index';
...
@ccclass('StarlightV2_RollerPanel')
export class StarlightV2_RollerPanel extends BaseComponent<IState, IProps, IEvent> {
	start() {

	}

	private rollerViewModelArr: StarlightV2RollerViewModel[] = [];
	...
	private initRoller() {
		for (let i = 0; i < config.columnCount; i++) {
			...
			const roller = new StarlightV2RollerViewModel().mountView(column).appendTo(this.node).connect();
	...
	private playStopRollSound() {
		this.scheduleOnce(() => {
			if (!this.props.isManualSpeed && !this.props.isSpeed) {
				// starlightv2_Audio.playOneShot(SoundPathDefine.DROP_BOTTOM)
			} else {
				starlightv2_Audio.playOneShot(SoundPathDefine.QUICK_STOP)
			}
		}, 0.3)
	}
	...
	private rollEndHandle(rollerLaunchResult: RollerLaunchResult) {
		...
		mainViewModel.comp.setMaskEnable(false);
	...
	private playSingleLabelAnimation(parentNode: Node, labelNode: Node, endPos: Vec3, done) {
		...
		tween(labelTemp).delay(delayTime)
			...
			.call(() => {
				starlightv2_Audio.playOneShot(SoundPathDefine.WILD_FLASH)
			})
	...
	private clearIconHandle(index: number, clearRoundDatas: ClearRoundData[], done) {
		...
		if (this.isSubGame1()) {
			starlightv2_Audio.playOneShot(SoundPathDefine.SP_WIN)
		} else {
			starlightv2_Audio.playOneShot(SoundPathDefine.SP_WIN_SHOW)
		}
	...
	private changeToMainGame(gameType: GameType, rollerLaunchResult: RollerLaunchResult) {
		...
		mainViewModel.enterExitSubGame({
			...
			params: [rollerLaunchResult.dl.si[0].freeGameAmount, this.props.gameTypeInfo.totalCount],
		}, () => {
			// 播放切换游戏动画
			this.dispatch(updateSubGameAnimationPlayInfo({
				gameType: gameType,
