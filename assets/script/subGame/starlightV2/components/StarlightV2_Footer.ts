...
import { starlightv2_Audio, mainViewModel } from '../index';
...
@ccclass('StarlightV2_Footer')
export class StarlightV2_Footer extends BaseComponent<IState, IProps, IEvent> {
	start() {
	...
	protected bindEvent(): void {
		...
		this.propertyNode.props_toggle_fast.on(Toggle.EventType.TOGGLE, (e: Toggle) => {
			starlightv2_Audio.playOneShot(SoundPathDefine.BUTTONCLICK)
		...
		this.propertyNode.props_min_btn.on(Node.EventType.TOUCH_END, () => {
			...
			starlightv2_Audio.playOneShot(SoundPathDefine.MIN_COIN)
		...
		this.propertyNode.props_max_btn.on(Node.EventType.TOUCH_END, () => {
			...
			starlightv2_Audio.playOneShot(SoundPathDefine.MAX_COIN)
			this.dispatch(updatePositionId(this.props.positionId + 1));
			this.updateSprMoney();
		})
