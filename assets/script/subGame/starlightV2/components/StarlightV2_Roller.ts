...
import { StarlightV2Icon } from './StarlightV2_Icon';
import { mainViewModel, starlightv2_Audio } from '../index';
...
@ccclass('StarlightV2_Roller')
export class StarlightV2_Roller extends BaseComponent<IState, IProps, IEvent> {
	start() {
	...
	/**滚轴上的图标，包含看不见的 */
	private iconArr: StarlightV2Icon[] = [];
	/**滚动结束回调方法 */
	...
	private playWinSound(roundIndex: number) {
		...
		starlightv2_Audio.playOneShot(soundStr)
	}

	/**播放scatter的动画，然后进入小游戏弹窗 */
	public playScatterAnimation(done) {
		const scatterIcons: StarlightV2Icon[] = [];
		for (let i = 0; i < this.showIconCount; i++) {
	...
	private rollOverHandle(rollerLaunchResult: RollerLaunchResult) {
		...
		if (!this.isSpeed) {
			starlightv2_Audio.playOneShot(SoundPathDefine.SINGLE_ICON_FALL)
		}
	...
	private addIcon(done) {
		...
		this.scheduleOnce(() => {
			fillIndexs.forEach((fillIndex, i) => {
				...
				const iconObj = icon as StarlightV2Icon;
				...
				if (iconObj.isOddsIcon()) {
					...
					starlightv2_Audio.playOneShot(SoundPathDefine.WILD_SHOW);
				}
				...
				this.dropIcon(i, iconObj, index, nextIndex, () => {
					...
					if (this.deleteWaitPlayAnimationIcon(iconObj.getId())) {
						if (iconObj.isScatterIcon() && this.columnIndex < 3) {
							starlightv2_Audio.playOneShot(SoundPathDefine.scatter_flash1)
							this.randomPlayScatterSound()
						}
	...
	private randomPlayScatterSound() {
		const arr = [SoundPathDefine.vol_scatter1, SoundPathDefine.vol_scatter2, SoundPathDefine.vol_scatter3, SoundPathDefine.vol_scatter4,
		SoundPathDefine.vol_scatter5, SoundPathDefine.vol_scatter6, SoundPathDefine.vol_scatter7, SoundPathDefine.vol_scatter8];
	 ...
	 * @param isDelay 
	 */
	private dropIcon(orderIndex: number, icon: StarlightV2Icon, curIndex: number, nextindex: number, done, isDelay: boolean = false) {
		...
		tw.call(() => {
			if (nextindex === -1) {
				...
				!this.isSpeed && starlightv2_Audio.playOneShot(SoundPathDefine.SINGLE_ICON_FALL)
			}
			done && done();
	...
	/**播放闪电 */
	public playLightning(endPos: Vec3, icon: StarlightV2Icon, time: number) {
	...
	public getOddsLabelNodes() {
		const labelNodes: StarlightV2Icon[] = [];
		for (let i = 0; i < this.showIconCount; i++) {
			if (this.iconArr[i].isOddsIcon()) {
