...
export class GodWealthV2_DialogWin extends BaseComponent<
  IState,
  IProps,
  IEvent
> {
  ...
  protected bindEvent(): void {
    this.node.on(Node.EventType.TOUCH_END, () => {
      if (
        this.isFreeGameEntry() ||
        this.isBaoHeGameEntry() ||
        this.isFreeGameSettlement() ||
        this.isBaoHeGameSettlement()
      ) {
        this.node.destroy();
  ...
  private isFreeGameEntry() {
    return (
      ...
      this.props.dialogInfo.dialogType === DialogType.FREE_GAME_ENTRY
    );
  }
  ...
  private isBaoHeGameEntry() {
    return (
      this.props.dialogInfo &&
      this.props.dialogInfo.dialogType === DialogType.BAO_HE_GAME_ENTRY
    );
  }
  ...
  private isFreeGameSettlement() {
    return (
      this.props.dialogInfo &&
      this.props.dialogInfo.dialogType === DialogType.FREE_GAME_SETTLEMENT
    );
  }
  ...
  private isBaoHeGameSettlement() {
    return (
      this.props.dialogInfo &&
      this.props.dialogInfo.dialogType === DialogType.BAO_HE_GAME_SETTLEMENT
    );
  }
    ...
    if (this.node && this.node.isValid) {
      if (key === "dialogInfo") {
        ...
        if (value.cur) {
          ...
          let lbl_des = this.propertyNode.props_jackpot
            .getChildByName("lbl_des")
            .getComponent(Label);
          if (this.props.dialogInfo.dialogType === DialogType.FREE_GAME_ENTRY) {
            ...
            this.props.dialogInfo.dialogType === DialogType.SUPER_WIN
          ) {
            this.showJackpot();
            ...
            this.props.dialogInfo.dialogType ===
  ...
  private showFreeEntry() {
    ...
    const labelNode = getNodeByNameDeep(
      "Label_Number",
      this.propertyNode.props_free_spins
    );
  ...
  private showBigWin() {
    ...
    const numberLabel = getNodeByNameDeep(
      "number",
      this.propertyNode.props_bigWin
    );
    this.startStepNumber(
      numberLabel.getComponent(Label),
      0,
      this.props.dialogInfo.params[0],
      () => {
        this.detryCloseWindow();
      }
    );
  ...
  private showJackpot() {
    ...
    const numberLabel = getNodeByNameDeep(
      "number",
      this.propertyNode.props_jackpot
    );
    this.startStepNumber(
      numberLabel.getComponent(Label),
      0,
  ...
  private showMegaWin() {
    ...
    const numberLabel = getNodeByNameDeep(
      "number",
      this.propertyNode.props_megaWin
    );
    this.startStepNumber(
      numberLabel.getComponent(Label),
      0,
  ...
  private showSuperWin() {
    ...
    const numberLabel = getNodeByNameDeep(
      "number",
      this.propertyNode.props_superWin
    );
    this.startStepNumber(
      numberLabel.getComponent(Label),
      0,
  ...
  private showFreeSettlement() {
    ...
    const numberLabel = getNodeByNameDeep(
      "Label_Number",
      this.propertyNode.props_freeGame_win
    );
  ...
  private showBaoHeSettlement() {
    ...
    const numberLabel = getNodeByNameDeep(
      "Label_Number",
      this.propertyNode.props_baoHeGame_win
    );
    this.startStepNumber(
      numberLabel.getComponent(Label),
      0,
