...
export class GodWealthV2_Header extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
    ...
    this.propertyNode.props_btn_up_menu.on(Node.EventType.TOUCH_END, () => {
      ...
      if (!this.isShowMenu) {
        ...
        this.propertyNode.props_menu_panel.active =
          !this.propertyNode.props_menu_panel.active;
    ...
    this.propertyNode.props_menu_panel.on(
  ...
  protected useProps(key: keyof IProps, value: { pre: any; cur: any }) {
    if (this.node && this.node.isValid) {
      if (key === "gold") {
        if (this.init) {
          this.propertyNode.props_Moeny_Label.string = Number(
            value.cur
          ...
          stepNumber.setFlyNode(
            this.propertyNode.props_Moeny_Label.node.parent,
            this.propertyNode.props_Moeny_Label.node,
            20
          );
          stepNumber.start();
        }
