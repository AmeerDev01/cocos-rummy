import { _decorator, Button, Color, Label, Node, PageView, Toggle } from "cc";
...
export class GodWealthV2_helpPanel extends BaseComponent<
  IState,
  IProps,
  IEvent
> {
  ...
  protected bindEvent(): void {
    this.propertyNode.props_btn_close.node.on(
    ...
    initToggle(
      this.propertyNode.props_Toggle_Panel,
      this.node,
      new GodWealthV2_helpPanel.EventHandler(),
  ...
  protected channelToggleCallback(event: Event, customEventData: string) {
    ...
    for (let i = 0; i < toggleArr.length; i++) {
      if (toggleArr[i].getComponent(Toggle).isChecked) {
        ...
        toggleArr[i].getChildByName("Panel_Label").getComponent(Label).color =
          new Color().fromHEX("#942600");
      } else {
        toggleArr[i].getChildByName("Panel_Label").getComponent(Label).color =
          new Color().fromHEX("#F8C55C");
      }
    }
  }
  ...
  private showIndex(num) {
    if (!this.propertyNode) return;
    let arr = this.propertyNode.props_indicator.children.filter((v) =>
      v.name.startsWith("props_indicator_toggle")
    );
    arr.forEach((item, i) => {
      if (i === num) {
