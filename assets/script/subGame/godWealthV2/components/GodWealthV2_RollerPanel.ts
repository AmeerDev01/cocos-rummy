...
import {
  changeGame,
  updateGold,
  updateSubGameAnimationPlayInfo,
  updateWinloss
...
export class GodWealthV2_RollerPanel extends BaseComponent<
  IState,
  IProps,
  IEvent
> {
  ...
  private initRoller() {
    for (let i = 0; i < config.columnCount; i++) {
      ...
      const roller = new GodWealthV2RollerViewModel()
        .mountViewNode(column)
        .appendTo(this.propertyNode.props_column_list)
        .connect();
  ...
  private rollEndHandle() {
    ...
    const winLine = this.startPlayWinIcon(
      config.normalRollOption.winIconFlickerTime,
      () => {}
    );
    this.playWildAudio();
    if (!auto || winLine === 0) {
  ...
  private updateWinloss() {
    const si =
      cacheData.rollerLaunchResult && cacheData.rollerLaunchResult.dl.si[0];
    const roundTw = si.tw;
    if (this.isMain()) {
      if (roundTw <= 0) {
        return;
      ...
      if (!this.isSubGame2()) {
        ...
        this.dispatch(
          updateWinloss(cacheData.rollerLaunchResult.dl.si[0].freeGameAmount)
        );
      }
    }
  }

  private updateRollerStatusEnd(
    bigWinConfig,
    auto: boolean,
    isSubGameLastPoint: boolean
  ) {
    ...
    if (!isEnterSubGame2 && bigWinConfig) {
        ...
        if (isChangeGame) {
          if (
            ...
            this.playBeetleAnimation(() => {
              this.dispatch(
                updateWinloss(
                  cacheData.rollerLaunchResult.dl.si[0].freeGameAmount
                )
              );
              this.dispatch(
                updateGold(cacheData.rollerLaunchResult.dl.si[0].bl)
              );
      ...
      this.scheduleOnce(() => {
        ...
        const winloss =
          this.isSubGame2() && isSubGameLastPoint ? si.freeGameAmount : si.tw;
  ...
  private playBeetleAnimation(done) {
    ...
    const endPos = this.node
      .getComponent(UITransform)
      .convertToNodeSpaceAR(mainViewModel.getGoodLuckPos());
    let count = 0;
    godWealthV2_Audio.playOneShot(SoundPathDefine.SUN_GOD, 1.0, false);
    ParticleNodes.forEach((p, i) => {
      const startPos = this.node
        .getComponent(UITransform)
        .convertToNodeSpaceAR(p.getWorldPosition());
      ...
      mainViewModel.showDialogWin(
        {
          dialogType:
            this.props.gameTypeInfo.currGameType === GameType.SUBGAME1
              ? DialogType.FREE_GAME_ENTRY
              : DialogType.BAO_HE_GAME_ENTRY,
      ...
      mainViewModel.showDialogWin(
        {
          dialogType:
            this.props.gameTypeInfo.viewGameType === GameType.SUBGAME1
              ? DialogType.FREE_GAME_SETTLEMENT
              : DialogType.BAO_HE_GAME_SETTLEMENT,
  ...
  private getBigWinConfig(magnification: number) {
    return config.winning.find(
      (v) => v.minRate <= magnification && magnification <= v.maxRate
    );
  }
  private getIsSpeed() {
    if (this.isSubGame2()) {
  ...
  public getScatterCount(endColumnIndex: number) {
    ...
    endColumnIndex =
      endColumnIndex >= this.rollerViewModelArr.length
        ? this.rollerViewModelArr.length
        : endColumnIndex;
  ...
  private changeGame(gameType: GameType) {
    const columnListLayout =
      this.propertyNode.props_column_list.getComponent(Layout);
    const isSubGame2 = gameType === GameType.SUBGAME2;
    const isMain = gameType === GameType.MAIN;
    if (isMain || isSubGame2) {
      this.node.position = new Vec3(
        config.normalRollOption.rollerWindowX,
        config.normalRollOption.rollerWindowY
      );
      columnListLayout.spacingX = config.normalRollOption.columnInterval;
    } else {
      this.node.position = new Vec3(
        config.subGameRollOption.rollerWindowX,
        config.subGameRollOption.rollerWindowY
      );
