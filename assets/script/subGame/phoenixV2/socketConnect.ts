...
export let phoenixWebSocketDriver: WebSocketDriver<SKT_MAG_TYPE> = null
export default () => {
  ...
  return new Promise((resolve, reject) => {
    initConfig().then(() => {
      ...
      WebSocketStarter.Instance().initSocket().then(() => {
        phoenixWebSocketDriver = new WebSocketDriver<SKT_MAG_TYPE>(gameId, gameHost)
        phoenixWebSocketDriver.filterData = (data,source) => {
        ...
        //   return false
        // })
        resolve(phoenixWebSocketDriver)
      })
    }).catch((e) => {
      reject(e)
    })
...
export const removeInstance = () => {
  phoenixWebSocketDriver && phoenixWebSocketDriver.logoutGame(SKT_MAG_TYPE.EXIT);
}