...
@ccclass('PhoenixV2_SingleIconRoller')
export class PhoenixV2_SingleIconRoller extends BaseComponent<IState, IProps, IEvent> {
	start() {
	...
	private getRollSpeed() {
		...
		if (this.rollCount <= this.rollMaxCount) {
			this.rollCount === this.rollMaxCount && this.addLastJoinIconIndex(this.stopIconIndex, 3);
	...
	private rollOverHandle(endIcon: PhoenixV2Icon) {
		this.scheduleOnce(() => {
