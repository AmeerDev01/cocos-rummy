...
export class Hall_LoginDialogV2 extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
    ...
    /**注册 */
    this.propertyNode.props_btn_daftarAkun.node.on(
      Node.EventType.TOUCH_END,
      () => this.reg()
    );
    /**登录 */
    this.propertyNode.props_btn_login.node.on(Node.EventType.TOUCH_END, () =>
      this.login()
    );
    /**忘记密码 */
    this.propertyNode.props_btn_login_forget.node.on(
      Node.EventType.TOUCH_END,
      () => {
  ...
  protected bindUI(): void {
    this.captchaGenerator = new CaptchaGenerator(
      this.propertyNode.props_spr_kode,
      4
    );
    if (sys.localStorage.getItem("loginName")) {
      this.propertyNode.props_input_akun.string =
        sys.localStorage.getItem("loginName");
    }
  }
  /**确认登录 */
  private login() {
    ...
    new InputValidator()
      .begin()
      .isLocalPhoneNumber(loginName)
      .isEmtry(
        password,
        lang.write(
          (k) => k.HallModule.passwordEmptyError,
          {},
          { placeStr: "请输入密码" }
        )
      )
      ...
      .done(() => {
        ...
        if (valodateCode.trim().length === 0) {
          global.hallDispatch(
            addToastAction({
              content: lang.write(
                (k) => k.HallModule.captchaEmptyError,
                {},
                { placeStr: "请输入验证码" }
              )
            })
        ...
        if (
          valodateCode.toLocaleLowerCase() !==
          this.captchaGenerator.captcha.toLocaleLowerCase()
        ) {
          this.captchaGenerator.generate();
      ...
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._LOGIN_DIALOG_REG_V2,
            Prefab
          )
        ).source
      )
