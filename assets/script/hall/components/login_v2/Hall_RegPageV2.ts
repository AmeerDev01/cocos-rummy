...
export class Hall_RegPageV2 extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
    ...
    /**发送验证码 */
    this.propertyNode.props_button_send_code.node.on(
        ...
        new InputValidator()
          ...
          .done(() => {
            ...
            this.events
              ...
              .then((code) => {
                this.propertyNode.props_input_smsCode.getComponent(
                  EditBox
                ).string = code;
              });
          });
    ...
    this.propertyNode.props_btn_ok.node.on(Node.EventType.TOUCH_END, () => {
      ...
      const verificationCode =
        this.propertyNode.props_input_smsCode.string.trim();
      let success = false;
      new InputValidator()
        .begin()
        ...
        .isEmtry(
          password_re,
          lang.write(
            (k) => k.HallModule.confirmPasswordEmptyError,
            {},
            { placeStr: "请输入确认密码" }
          )
        )
