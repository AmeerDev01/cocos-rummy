...
export class Hall_ActivityPanel extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
        ...
        if (item.gameId === ActivityType.TURNPLATE) {
          ...
          if (this.turntableViewModel) {
            ...
            hallWebSocketDriver.sktMsgListener.add(
              SKT_MAG_TYPE.TURNTABLEDATA,
              "activity",
              (data, error) => {
                if (error) return;
                if (data[0]) {
                  this.node &&
                    (this.turntableViewModel = new TurntableViewModel()
                      .mountView(
                        sourceManageSeletor().getFile(
                          PrefabPathDefine.HELL_ACTIVITY_TURNTABLEPANL
                        ).source
                      )
                      .appendTo(this.node)
                      .setEvent({
          ...
          this.turntableViewModel &&
            (this.turntableViewModel.comp.node.active = false);
          ...
          item.pictureUrl &&
            assetManager.loadRemote(
              ...
              (err, asset: ImageAsset) => {
                this.propertyNode &&
                  (this.propertyNode.props_loading_img.active = false);
                if (
                  this.propertyNode &&
                  !err &&
                  lastQeqPicUrl === asset.nativeUrl
                ) {
                  this.propertyNode.props_img_content.active = true;
                  this.propertyNode.props_img_content.getComponent(
                    Sprite
                  ).spriteFrame = SpriteFrame.createWithImage(asset);
                }
                this.propertyNode &&
                  (this.propertyNode.props_jumpTo_Telegram.active =
                    item.gameId === ActivityType.TELEGRAM);
                if (err) {
                  global.hallDispatch(
                    addToastAction({
                      content: lang.write(
    ...
    this.propertyNode.props_img_content.on(Node.EventType.TOUCH_END, () => {
      ...
      const detailUrl =
        this.props.activityList[this.state.chooseActivityIndex].detailUrl;
      const gameId =
        this.props.activityList[this.state.chooseActivityIndex].gameId;
      ...
      if (gameId !== null && gameId !== -1) {
        ...
        } else if (gameId === ActivityType.TURNPLATE) {
          // 返佣
        } else {
          ...
          if (hallGameGate) {
            const gateViewModel =
              baseBoardView.mainPanelViewModel.comp.gateViewModelList.find(
                (vm) =>
                  vm.comp.props.gamesIds.indexOf(hallGameGate.gameId) !== -1
              );
            ...
            global.hallDispatch(
              addToastAction({
                content: lang.write(
                  (k) => k.HallModule.HallGameLoading,
                  {
                    id: this.props.activityList[this.state.chooseActivityIndex]
                      .gameId
                  },
                  {
                    placeStr:
                      "无效的gameId:${this.props.activityList[this.state.chooseActivityIndex}"
                  }
                ),
                type: ToastType.ERROR
              })
    ...
    initToggle(
      this.propertyNode.props_ToggleGroup,
      this.node,
      new Hall_ActivityPanel.EventHandler(),
      "Hall_ActivityPanel",
      "toggleCallback"
    );
  }
  protected useProps(key: keyof IProps, value: { pre: any; cur: any }) {
    ...
    if (key === "activityList") {
      this.props.activityList.length &&
        (this.propertyNode.props_empty.node.active = false);
      this.propertyNode.props_ToggleGroup.removeAllChildren();
      this.props.activityList.forEach((item, i) => {
        const itemNode = instantiate(template);
        itemNode.name = "act_" + item.id;
        itemNode.getChildByName("Label_off").getComponent(Label).string =
          item.name;
        itemNode
          .getChildByName("Checkmark")
          .getChildByName("Label_in")
          .getComponent(Label).string = item.name;
        itemNode.getChildByName("unread").active =
          this.events.getItemUnreadStatus(item.id);
  ...
  private setToggleItemReadStatus(item: ActivityItem, isRead: boolean = true) {
    ...
    const itemNode = this.propertyNode.props_ToggleGroup.children.find(
      (v) => v.name === "act_" + item.id
    );
  ...
  protected onDestroy(): void {
    hallWebSocketDriver.sktMsgListener.remove(
      SKT_MAG_TYPE.TURNTABLEDATA,
      "activity"
    );
  }
