...
export interface IEvent {
  ...
  transferCapital?: (
    amount: number,
    direction: TrasferDirectionType
  ) => Promise<boolean>;
  /**设置银行密码 */
  setBankPassword?: (
...
export class Hall_BankPanel extends BaseComponent<IState, IProps, IEvent> {
  ...
  public events: IEvent = {
    ...
    verifyPwd: (pwd: string, isToadyAutoLogin: boolean) =>
      new Promise((resolve) => resolve(false)),
    transferCapital: (amount: number, direction: TrasferDirectionType) =>
      new Promise((resolve) => resolve(false)),
    setBankPassword: (loginPassword: string, safePassword: string) =>
      new Promise((resolve) => resolve(false))
  };
  ...
  protected bindEvent(): void {
    /**关闭验证界面 */
    this.propertyNode.props_btn_close_verify.on(
    ...
    this.propertyNode.props_btn_sure_verify.on(Node.EventType.TOUCH_END, () => {
      const str =
        this.propertyNode.props_intput_phone_phoneNum.getComponent(
          EditBox
        ).string;
      const isToadyAutoLogin =
        this.propertyNode.props_Toggle_bank_goin.getComponent(Toggle).isChecked;
      new InputValidator()
        .begin()
        .isNotEmpty(str)
        .done(() => {
    ...
    this.propertyNode.props_set_btn_sure.on(Node.EventType.TOUCH_END, () => {
      const str =
        this.propertyNode.props_intput_sandi_pwd.getComponent(EditBox).string;
      const str2 =
        this.propertyNode.props_intput_bank_set_pwd.getComponent(
          EditBox
        ).string;
      new InputValidator()
        .begin()
        .isNotEmpty(str)
        .isCharLength([6, 12], str2)
        .done(() => {
    ...
    this.propertyNode.props_btn_bank_setor.on(Node.EventType.TOUCH_END, () => {
      this.trasfer(
        this.propertyNode.props_EditBox_meng_goldNum.getComponent(EditBox)
          .string,
        TrasferDirectionType.SETOR
      );
    });
    this.propertyNode.props_btn_bank_meng.on(Node.EventType.TOUCH_END, () => {
      this.trasfer(
        this.propertyNode.props_EditBox_meng_goldNum.getComponent(EditBox)
          .string,
        TrasferDirectionType.MENGAMBILF
      );
    });
    this.propertyNode.props_btn_history.node.on(
      Node.EventType.TOUCH_END,
      () => {
        //打开记录
        sys.openURL(
          config.goldRecordUrl +
            `?token=${sys.localStorage.getItem("token")}&memberId=${
              getStore().getState().memberInfo.memberId
            }&env=${getIsTest() ? "test" : "pro"}`
        );
      }
    );
  }
    ...
    new InputValidator()
      ...
      .done(() => {
        this.events.transferCapital(+amount, direction).then(() => {
          this.propertyNode.props_EditBox_meng_goldNum.getComponent(
            EditBox
          ).string = "";
        });
      });
  ...
  protected useProps(key: keyof IProps, value: { pre: any; cur: any }) {
    ...
    key === "memberAssetSafeGoldPieces" &&
      (this.propertyNode.props_Label_akun_num.getComponent(Label).string =
        value.cur.formatAmountWithCommas());
  }
  ...
  protected bindUI(): void {
    this.useState(
      (key, value) => {
        ...
        value.cur === ShowModelType.VERIFY &&
          (this.propertyNode.props_bank_goin.active = true);
        value.cur === ShowModelType.MAIN_PANEL &&
          (this.propertyNode.props_main_panel.active = true);
        value.cur === ShowModelType.SET_PWD &&
          (this.propertyNode.props_bank_set.active = true);
      },
      ["showModel"]
    );
