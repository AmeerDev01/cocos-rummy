...
export class Hall_VipMain extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
    this.propertyNode.props_btn_vip_close.node.on(
    ...
    this.propertyNode.props_btn_aktifkan.node.on(
      Node.EventType.TOUCH_END,
      () => {
        global.openShop();
      }
  ...
  protected bindUI(): void {
    this.locateInitPosition = new Vec3(
      this.propertyNode.props_node_locate.position.x,
      this.propertyNode.props_node_locate.position.y
    );
  }
  ...
  protected useProps(
    ...
    if (key === "vipBonusInfo") {
      if (!value.cur) return;
      this.getUserVipGoldNd(this.propertyNode.props_spr_promotion).string =
        value.cur.myUpgradeBonus + "";
      this.getUserVipGoldNd(this.propertyNode.props_spr_daily).string =
        value.cur.myDayBonus + "";
      this.getUserVipGoldNd(this.propertyNode.props_spr_weekly).string =
        value.cur.myWeeklyBonus + "";
      this.getUserVipGoldNd(this.propertyNode.props_spr_monthly).string =
        value.cur.myMonthlyBonus + "";

      this.getOtherVipGoldNd(this.propertyNode.props_spr_promotion).string =
        value.cur.clickUpgradeBonus + "";
      this.getOtherVipGoldNd(this.propertyNode.props_spr_daily).string =
        value.cur.clickDayBonus + "";
      this.getOtherVipGoldNd(this.propertyNode.props_spr_weekly).string =
        value.cur.clickWeeklyBonus + "";
      this.getOtherVipGoldNd(this.propertyNode.props_spr_monthly).string =
        value.cur.clickMonthlyBonus + "";
  ...
  private initVipToggle() {
    ...
    vipMap.forEach((v) => {
      const vipLevel = v[0];
      const vipToggleItem = instantiate(
        this.propertyNode.props_vip_toggle_item
      );
      getNodeByNameDeep("label_vip_unchecked", vipToggleItem).getComponent(
        Label
      ).string = "VIP" + vipLevel;
      getNodeByNameDeep("label_vip_checked", vipToggleItem).getComponent(
        Label
      ).string = "VIP" + vipLevel;
      getNodeByNameDeep("label_achieve_vip", vipToggleItem).getComponent(
        Label
      ).string = "VIP" + vipLevel;
      getNodeByNameDeep("spr_achieve_vip", vipToggleItem).active =
      ...
      if (isChecked) {
        ...
        if (userVipLevel > 10) {
          this.propertyNode.props_ScrollView_vip
  ...
  private updateLevelBar() {
    ...
    const percent =
      vipLevel < maxVipLevel
        ? (vipLevelExperience / vipMap.find((v) => v[0] === vipLevel + 1)[1]) *
          0.5
        : 0;
    const barValue =
      vipLevel < maxVipLevel ? (vipLevel === 0 ? 0 : 0.5) + percent : 1;
    ...
    locateNode.getChildByName("label_suffer").getComponent(Label).string =
      userTotalRechange.formatAmountWithCommas();
      ...
      .to(
        ...
        { progress: barValue > 1 ? 1 : barValue },
        {
          onUpdate: () => {
            ...
            locateNode.setPosition(
              new Vec3(locateInitPosition.x + barWidth, locateInitPosition.y)
            );
          }
        }
      )
  ...
  private updateBarLevelLabel(vipLevel: number) {
    ...
    this.propertyNode.props_pro_levelbar.children
      .filter((v) => v.name.startsWith("spr_level_"))
      .forEach((v, i) => {
        v.getChildByName("Label_level_vip").getComponent(Label).string =
          levels[i] + "";
      });
  }
  ...
  private async updateDiamondIcon(vipLevel: number) {
    ...
    this.propertyNode.props_diamond_icon.getComponent(Sprite).spriteFrame = (
      await sourceManageSeletor().getFileAsync(iconFile, SpriteFrame)
    ).source;
    this.propertyNode.props_me_vip_level_label.getComponent(Label).string =
      vipLevel + "";
  }
  ...
  private updateRemindInfo() {
    ...
    if (nextLevel >= maxVipLevel) {
      this.propertyNode.props_remind_next_level_gold.getComponent(
        Label
      ).string = "0";
      this.propertyNode.props_remind_vip.getComponent(Label).string =
        "VIP" + maxVipLevel;
      return;
    }
    const vipInfo = vipMap.find((v) => v[0] === nextLevel);
    const experience = vipInfo[1] - this.props.memberInfo.vipLevelExperience;
    this.propertyNode.props_remind_next_level_gold.getComponent(Label).string =
      experience.formatAmountWithCommas();
    this.propertyNode.props_remind_vip.getComponent(Label).string =
      "VIP" + nextLevel;
  }
  ...
  private updateRemindtextInfo(vipInfo: number[]) {
    ...
    let userVipLevel = this.props.memberInfo.vipLevel;
    this.getOtherVip(this.propertyNode.props_spr_promotion).string =
      vipLevel + "";
    ...
    this.getOtherVip(this.propertyNode.props_spr_monthly).string =
      vipLevel + "";

    this.getUserVip(this.propertyNode.props_spr_promotion).string =
      "VIP" + userVipLevel;
    this.getUserVip(this.propertyNode.props_spr_daily).string =
      "VIP" + userVipLevel;
    this.getUserVip(this.propertyNode.props_spr_weekly).string =
      "VIP" + userVipLevel;
    this.getUserVip(this.propertyNode.props_spr_monthly).string =
      "VIP" + userVipLevel;

    if (vipLevel <= userVipLevel) {
      this.propertyNode.props_remindtext_next_label_gold.getComponent(
        Label
      ).string = "0";

      if (vipLevel < userVipLevel) {
        //升级奖
        this.getAvailableNode(this.propertyNode.props_spr_promotion).active =
          false;
        ...
        this.getLockNode(this.propertyNode.props_spr_promotion).getChildByName(
          "spr_vip_lock"
        ).active = false;
        ...
        this.getLockNode(this.propertyNode.props_spr_daily).getChildByName(
          "spr_vip_lock"
        ).active = false;
        //每周
        this.getAvailableNode(this.propertyNode.props_spr_weekly).active =
          false;
        ...
        this.getLockNode(this.propertyNode.props_spr_weekly).getChildByName(
          "spr_vip_lock"
        ).active = false;

        //每月
        this.getAvailableNode(this.propertyNode.props_spr_monthly).active =
          false;
        ...
        this.getLockNode(this.propertyNode.props_spr_monthly).getChildByName(
          "spr_vip_lock"
        ).active = false;
      ...
      this.propertyNode.props_remindtext_next_label_gold.getComponent(
        ...
        this.getTotalRechange(vipLevel) - this.getUserTotalRechange()
      ).formatAmountWithCommas();

      //升级奖
      this.getAvailableNode(this.propertyNode.props_spr_promotion).active =
        false;
      this.getReceivedNode(this.propertyNode.props_spr_promotion).active =
        false;
      this.getLockNode(this.propertyNode.props_spr_promotion).active = true;
      this.getLockNode(this.propertyNode.props_spr_promotion).getChildByName(
        "spr_vip_lock"
    ...
    this.propertyNode.props_remindtext_vip.getComponent(Label).string =
      "VIP" + vipLevel;
  }
  ...
  private getUserTotalRechange() {
    return (
      this.getTotalRechange(this.props.memberInfo.vipLevel) +
      this.props.memberInfo.vipLevelExperience
    );
  }
  ...
  private getVipBonus(vipLevel) {
    ...
    fetcher
      .send(ApiUrl.GET_VIP_BONUS, { vipLevel: vipLevel })
      .then(async (rsp) => {
        this.dispatch(setLoadingAction({ isShow: false, flagId: "vip" }));
