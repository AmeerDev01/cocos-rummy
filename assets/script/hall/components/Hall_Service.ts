...
export class Hall_Service extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
    this.propertyNode.props_btn_service_close.node.on(
      Node.EventType.TOUCH_END,
      () => {
        this.events.onClosePanel();
      }
    );
    this.propertyNode.props_btn_service_browser.node.on(
      Node.EventType.TOUCH_END,
      () => {
    ...
    this.propertyNode.props_btn_service_officialWebsite.node.on(
      Node.EventType.TOUCH_END,
      () => {
