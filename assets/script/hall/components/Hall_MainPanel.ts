...
export interface IEvent {
  onToggleGameTypeHandler?: (
    gameType: SubGameType,
    gameGateViewModelList: SubGameGateViewModel[]
  ) => void;
  /**点击游戏触发 */
...
export class Hall_MainPanel extends BaseComponent<IState, IProps, IEvent> {
  ...
  callback(event: Event, customEventData: string) {
    ...
    this.scheduleOnce(() => {
      arrowDisplay(
        this.propertyNode.props_ScrollView_game_icon.getComponent(ScrollView),
        this.propertyNode.props_btn_left,
        this.propertyNode.props_btn_right,
        40
      );
    }, 0.1);
  }
  ...
  protected bindEvent(): void {
    initToggle(
      this.propertyNode.props_ToggleGroup_type,
      this.node,
      new Hall_MainPanel.EventHandler(),
      "Hall_MainPanel",
      "callback"
    );
    ...
    this.propertyNode.props_btn_down_referral.on(
    ...
    this.propertyNode.props_btn_down_withdrawal.on(
      Node.EventType.TOUCH_END,
      () => {
        this.isDebouncerAsync(() => {
    ...
    this.propertyNode.props_btn_down_shopGift.on(
      Node.EventType.TOUCH_END,
      () => {
        this.isDebouncerAsync(() => {
    ...
    this.propertyNode.props_btn_down_service.on(
      Node.EventType.TOUCH_END,
      () => {
        this.isDebouncerAsync(() => {
    ...
    this.propertyNode.props_ScrollView_game_icon.on(
    ...
    this.propertyNode.props_btn_right.on(Node.EventType.TOUCH_END, () => {
      const scrollView =
        this.propertyNode.props_ScrollView_game_icon.getComponent(ScrollView);
      const wrapWodth =
        this.propertyNode.props_ScrollView_game_icon.getComponent(
          UITransform
        ).width;
      tween(scrollView.content)
        .to(0.1, {
          position: new Vec3(
            -scrollView.content.getComponent(UITransform).width + wrapWodth,
            0,
            0
          )
        })
  ...
  protected useProps(
    ...
    key === "nickName" &&
      (this.propertyNode.props_Label_name.string = value.cur + "");
    ...
    if (key === "memberAssetGoldPieces") {
      if (this.initGame) {
        this.propertyNode.props_Label_gold.getComponent(Label).string = Number(
          value.cur
        ...
        const stepNumber = new StepNumber(value.pre, value.cur, (num) => {
          if (this.node && this.node.isValid) {
            this.propertyNode.props_Label_gold.getComponent(Label).string =
              Number(value.cur).formatAmountWithCommas();
          }
        });
        stepNumber.setFlyNode(
          this.propertyNode.props_Label_gold.parent,
          this.propertyNode.props_Label_gold,
          30
        );
        stepNumber.start();
      }
    ...
    if (key === "avatarIndex") {
      bundleCommon.load(
        `resource/head/head_circle_${value.cur}/spriteFrame`,
        SpriteFrame,
        (err, sp) => {
          if (!err) {
    ...
    if (key === "vipLevel") {
      bundleCommon.load(
        `resource/vip/VIP${value.cur}/spriteFrame`,
        SpriteFrame,
        (err, sp) => {
          !err &&
            (this.propertyNode.props_spr_vip.getComponent(Sprite).spriteFrame =
              sp);
        }
      );
    }
    ...
    if (key === "UnreadMailNum") {
      this.propertyNode.props_btn_down_inbox.getChildByName(
        "props_inbox_red_dot"
      ).active = value.cur ? true : false;
  ...
  protected bindUI(): void {
    this.useState(() => {
      ...
      if (!isInit) {
        this.gateViewModelList.forEach(
          (item) => (item.viewNode.active = false)
        );
      }
      this.state.currGateQueue.queue.forEach((gamesIds) => {
        if (!isInit) {
          const target = this.gateViewModelList.find((item) =>
            equal(item.comp.props.gamesIds, gamesIds)
          );
      ...
      this.events.onToggleGameTypeHandler(
        this.state.subGameType,
        this.gateViewModelList
      );
    }, ["chooseTabName"]);
    this.setState({
    ...
    fetcher
      .send(ApiUrl.GAME_SWIPE, {}, "get", {})
      .then((data: any) => {
        if (!data || (data && !Array.isArray(data))) {
          global.hallDispatch(
            addToastAction({
              content: lang.write(
                (k) => k.BaseBoardModule.DataException,
                {},
                { placeStr: "数据异常" }
              ),
        ...
        data.forEach((item, i) => {
          assetManager.loadRemote(item.swiperUrl, (err, asset: ImageAsset) => {
            if (this.propertyNode && !err) {
              ...
              if (
                this.swiperDatas.length === data.length &&
                this.swiperDatas.length > 1
              ) {
    ...
    new MarqueeViewModel()
      .mountView(
        sourceManageSeletor("common").getFile(
          PrefabPathDefineCommon.MARQUEE_INSERT
        ).source
      )
      .appendTo(this.propertyNode.props_Marquee_node)
      .connect();
  ...
  private initGateModelView(scrollViewContent: Node, gamesIds: number[]) {
    const gateModelView = new SubGameGateViewModel()
      .mountView(
        sourceManageSeletor().getFile(PrefabPathDefine.SUB_GAME_GATE).source
      )
  ...
  private scrollPageViewListener() {
    this.propertyNode.props_PageView.node.on(
      PageView.EventType.SCROLL_ENDED,
      (pageView: PageView) => {
        ...
        const targetToggle = this.swiperDatas.find(
          (v) => v.name === page.name
        ).toggle;
  ...
  private scrollPageView() {
    ...
    this.propertyNode.props_PageView.scrollToPage(
      pageIndex === length - 1 ? 1 : ++pageIndex
    );
  }
  ...
  private addPageViewNode(index: number, insertIndex: number = -1) {
    ...
    node.active = true;
    insertIndex >= 0
      ? this.propertyNode.props_PageView.insertPage(node, insertIndex)
      : this.propertyNode.props_PageView.addPage(node);
  ...
  private getGetSubGameGateQueue(
    tabName: ToggleTabName,
    typeName: SubGameType
  ) {
    // 获取当前要显示的图标数据
    const subGameQueue = subGameGateQueue.find(
      (item) => `Toggle_${item.toggleName}` === tabName
    ).queue;
    this.setState({
      currGateQueue: {
        typeName,
  ...
  public setSubGameGate(gameId: number, progress: number, isShow?: boolean) {
    const gateViewModel = this.gateViewModelList.find(
      (item) => item.comp.props.gamesIds.indexOf(gameId) !== -1
    );
    gateViewModel.comp.setLoadingState(
      gameId,
      progress,
      isShow,
      isH5() ? 0.5 : 1
  ...
  private isDebouncerAsync(done) {
    Throttler.isDebouncerAsync("hall_entry", 1000, true, () => {}).then(
      (isPass) => {
        done && done();
      }
    );
