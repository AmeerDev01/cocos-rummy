...
export class Hall_WaterHelp extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
    this.propertyNode.props_btn_spr_shareHelp_close.node.on(
  ...
  protected bindUI(): void {
    this.propertyNode.props_web_help.url = `${
      config.backwaterHelpUrl
    }?token=${sys.localStorage.getItem("token")}&env=${
      getIsTest() ? "test" : "pro"
    }`;
  }
