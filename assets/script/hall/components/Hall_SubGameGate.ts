import { _decorator, assert, Button, Color, Component, instantiate, Label, Layout, Node, Prefab, ProgressBar, sp, Sprite, SpriteFrame, sys, UIOpacity, UITransform } from 'cc';
import { BaseComponent } from '../../base/BaseComponent';
import { config, HallGameGateType, subGameList } from '../config'
import { baseBoardView, bundleCommon, bundleHall, global, hallAudio, sourceManageSeletor } from '../index';
import { SkeletalPathDefine } from '../sourceDefine/skeletalDefine';
import BundleSplit from '../../utils/BundleSplit';
import { addToastAction, setLoadingAction, setSubGameInfoAction, setSubGameRunState, ToastType } from '../store/actions/baseBoard';
...
export class Hall_SubGameGate extends BaseComponent<IState, IProps, IEvent> {
	...
	private async renderGateIcon(gameId: number) {
		...
		if (gameId) {
			...
			skeletonComp.skeletonData = (await sourceManageSeletor()
	...
	private downLoadZipGame(hallGameGate: HallGameGateType) {
		...
		if (this.downLoadState(hallGameGate.gameId)) {
			...
			(new DownloadManagerGame()).DownloadMultiple([hallGameGate.bundleName],
				...
				() => {
					...
					this.dispatch(addToastAction({ content: lang.write(k => k.HallModule.GameDownLoadDone, { gameName: hallGameGate.gameName }, { placeStr: "游戏{gameName}下载完成，请轻击开始" }) }))
				},
				() => {
				...
				(e: any) => {
					...
					downLoadProgress.active = false
					this.dispatch && this.dispatch(addToastAction({ content: e, type: ToastType.ERROR }))
