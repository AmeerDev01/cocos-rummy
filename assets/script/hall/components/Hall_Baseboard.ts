...
export interface IProps {
	...
	loadPayload: { isShow: boolean, flagId: string, isAllowCloseLoading?: boolean }
	/**打开游戏 */
	...
	protected useProps(key: keyof IProps | '_setDone', value: { pre: any, cur: any }) {
		...
		if (key === "remainRetryCount") {
			...
			this.propertyNode.props_connect_tip.string = lang.write(k => k.WebSocketModule.socketRetryTimes,
				{ times: times[WebSocketStarter.Instance().maxReconnectTime - this.props.remainRetryCount] }, { placeStr: "连接断开，正在进行第{times}次重连..." })
		}
	}
