...
import {
  ...
  IProps as CNIprops,
  IEvent as CNIEvent
} from "./Hall_ChangeNickName";
import {
  Share_Info,
  IState as SHIState,
  IProps as SHIprops,
  IEvent as SHIEvent
} from "./Hall_PC_SubComp/Hall_PC_Share_Info";
import {
  Hall_PC_Share_member,
  IState as MBIState,
  IProps as MBIprops,
  IEvent as MBIEvent
} from "./Hall_PC_SubComp/Hall_PC_Share_member";
import { PrefabPathDefine } from "../sourceDefine/prefabDefine";
import { EffectType } from "../../utils/NodeIOEffect";
import UseSetOption from "../../utils/UseSetOption";
import { SoundPathDefine } from "../sourceDefine/soundDefine";
import BaseViewModel from "../../common/viewModel/BaseViewModel";
import { config, deviceInfo, levelMap, vipMap } from "../../config/config";
...
import {
  ...
  IProps as PBIprops,
  IEvent as PBIEvent
} from "./Hall_PC_SubComp/Hall_PC_BackWater";
...
export class Hall_PcMainPanel extends BaseComponent<IState, IProps, IEvent> {
  ...
  protected bindEvent(): void {
    //关闭
    this.propertyNode.props_btn_perfabs_close.on(
    ...
    //确认头像
    this.propertyNode.props_btn_change_tentu.on(
      Node.EventType.TOUCH_END,
      () => {
        this.events.memberInfoDone(
          this.props.memberInfo.gender,
          this.props.memberInfo.nickName,
          this.hasChooseAvatarIndex
        );
      }
    );
    //取消选择头像
    this.propertyNode.props_btn_change_batal.on(
      Node.EventType.TOUCH_END,
      () => {
    ...
    this.propertyNode.props_spr_sex.on(Node.EventType.TOUCH_END, () => {
      const sprittImg =
        this.propertyNode.props_spr_sex.getComponent(Sprite).spriteFrame
          .name === "icon_common_woman"
          ? "icon_common_man"
          : "icon_common_woman";
      bundleCommon.load(
        `resource/${sprittImg}/spriteFrame`,
        SpriteFrame,
        (err, sp) => {
          if (!err) {
            this.events.memberInfoDone(
              this.props.memberInfo.gender === 0 ? 1 : 0,
              this.props.memberInfo.nickName,
              this.props.memberInfo.avatarIndex,
              () => {
                this.propertyNode.props_spr_sex.getComponent(
                  Sprite
                ).spriteFrame = sp;
              }
            );
    ...
    // 	}
    // }, this);
    this.propertyNode.props_btn_set_bindPhone.on(
      Node.EventType.TOUCH_END,
      () => {
    ...
    /**打开升级框 */
    this.propertyNode.props_btn_set_ubasKataSandi.on(
      Node.EventType.TOUCH_END,
      () => {
        this.events.openChangePwdPanel();
      }
    );
    this.propertyNode.props_btn_set_switchAccount.on(
      Node.EventType.TOUCH_END,
      () => {
    ...
    this.propertyNode.props_btn_sure.node.on(Node.EventType.TOUCH_END, () => {
      ...
      this.propertyNode.props_img_vip_hlep.active = false;
      new InputValidator()
        .begin(this.propertyNode.props_EditBox_conver.string)
        .isNotEmpty()
        .done(() => {
          fetcher
            .send(ApiUrl.CODE_CONVER, {
            ...
            .then(() => {
              this.dispatch(
                addToastAction({
                  content: lang.write(
                    (k) => k.BaseBoardModule.operateDone,
                    {},
                    { placeStr: "操作成功" }
                  ),
    ...
    this.propertyNode.props_Toggle_set_music.on(
      "toggle",
      (toggle) => {
        let can = director.getScene();
        if (
          !can.getChildByName("Canvas") ||
          !can.getChildByName("Canvas").getChildByName("baseBoard") ||
          !can
            .getChildByName("Canvas")
            .getChildByName("baseBoard")
            .getChildByName("props_mainBoard")
        ) {
        } else {
          let nd = can
            .getChildByName("Canvas")
        ...
        UseSetOption.Instance().setOption(
          "music",
          toggle.target.getComponent(Toggle).isChecked
        );
      },
      this
    );
    this.propertyNode.props_Toggle_set_sound.on(
      "toggle",
      (toggle) => {
        let can = director.getScene();
        if (
    ...
    this.propertyNode.props_Toggle_set_vibration.on(
      "toggle",
      (toggle) => {
        UseSetOption.Instance().setOption(
          "vibration",
    ...
    initToggle(
      this.propertyNode.props_ToggleGroup_perfabs,
      this.node,
      new Hall_PcMainPanel.EventHandler(),
      "Hall_PcMainPanel",
      "containerEventHandler"
    );
    initToggle(
      this.propertyNode.props_ToggleGroup_profil,
      this.node,
      new Hall_PcMainPanel.EventHandler(),
      "Hall_PcMainPanel",
      "containerEventHandler1"
    );
    initToggle(
      this.propertyNode.props_ToggleGroup_head,
      this.node,
      new Hall_PcMainPanel.EventHandler(),
      "Hall_PcMainPanel",
      "containerEventHandler2"
    );
    initToggle(
      this.propertyNode.props_ToggleGroup_Share,
      this.node,
      new Hall_PcMainPanel.EventHandler(),
      "Hall_PcMainPanel",
    ...
    this.propertyNode.props_btn_level_help.node.on(
      Node.EventType.TOUCH_END,
      () => {
        this.propertyNode.props_img_level_hlep.active =
          !this.propertyNode.props_img_level_hlep.active;
        // bindCancleHelp()
      }
    );
    this.propertyNode.props_btn_vip_help.node.on(
      Node.EventType.TOUCH_END,
      () => {
        this.propertyNode.props_img_vip_hlep.active =
          !this.propertyNode.props_img_vip_hlep.active;
        // bindCancleHelp()
      }
    );

    this.propertyNode.props_EditBox_conver.node.on(
      Node.EventType.TOUCH_END,
      () => {
        this.propertyNode.props_img_level_hlep.active = false;
        this.propertyNode.props_img_vip_hlep.active = false;
  ...
  protected useProps(key: keyof IProps, value: { pre: any; cur: any }) {
    if (key === "memberInfo") {
      ...
      this.propertyNode.props_member_id.getComponent(Label).string =
        memberInfoNew.memberId;
      // this.propertyNode.props_EditBox_name.getComponent(EditBox).string = value.cur.nickName
      this.propertyNode.props_nick_name.getComponent(Label).string =
        memberInfoNew.nickName;
      this.propertyNode.props_Label_glod.getComponent(Label).string =
        memberInfoNew.memberAssetGoldPieces.formatAmountWithCommas();
      ...
      bundleCommon.load(
        `resource/head/head_circle_${memberInfoNew.avatarIndex}/spriteFrame`,
        SpriteFrame,
        (err, sp) => {
          !err &&
            (this.propertyNode.props_spr_head.getComponent(Sprite).spriteFrame =
              sp);
        }
      );
      bundleCommon.load(
        `resource/${
          memberInfoNew.gender === 0 ? "icon_common_woman" : "icon_common_man"
        }/spriteFrame`,
        SpriteFrame,
        (err, sp) => {
          !err &&
      ...
      this.propertyNode.props_ProgressBar_level_pro.getComponent(
        ProgressBar
      ).progress = memberInfoNew.levelExperience / levelItem[1];
      this.propertyNode.props_label_level_pro.getComponent(Label).string = `${(
        (memberInfoNew.levelExperience * 100) /
        levelItem[1]
      ).toFixed(0)}%`;

      let vipItem = vipMap.find((i) => i[0] === memberInfoNew.vipLevel + 1);
      if (!vipItem) vipItem = vipMap[vipMap.length - 1];
      this.propertyNode.props_ProgressBar_vip_pro.getComponent(
        ProgressBar
      ).progress = memberInfoNew.vipLevelExperience / vipItem[1];
      this.propertyNode.props_label_vip_pro.getComponent(
        Label
      ).string = `${memberInfoNew.vipLevelExperience}/${vipItem[1]}`;
      bundleCommon.load(
        `resource/vip/b_VIP${memberInfoNew.vipLevel}/spriteFrame`,
        SpriteFrame,
        (err, sp) => {
          !err &&
            this.propertyNode &&
            (this.propertyNode.props_word_informasi_vip.getComponent(
              Sprite
            ).spriteFrame = sp);
        }
      );
    ...
    if (key === "tourist") {
      this.propertyNode.props_btn_set_bindPhone.active =
        this.props.tourist !== 0;
      this.propertyNode.props_btn_set_ubasKataSandi.active =
        this.props.tourist === 0;
    }
  }
  ...
  protected bindUI(): void {
    ...
    this.useState(
      async (key, value) => {
        ...
        if (value.cur === 4) {
          ...
          this.share_InfoModel &&
            this.share_InfoModel.setProps({ isShow: false });
        }
        if (value.cur === 7) {
          this.loadBawahanData();
        } else {
          this.share_MemberModel &&
            this.share_MemberModel.setProps({ isShow: false });
        }
        this.propertyNode.props_layout_bag.active = value.cur === 5;
        //返水界面
        if (value.cur === 6) {
          !BackWaterVM &&
            (BackWaterVM = new BaseViewModel<
              Hall_PC_BackWater,
              PBIState,
              PBIprops,
              PBIEvent
              ...
              .mountView(
                (
                  await sourceManageSeletor().getFileAsync(
                    PrefabPathDefine.PC_BACK_WATER,
                    Prefab
                  )
                ).source
              )
          ...
          this.scheduleOnce(() => {
            this.isValid &&
              BackWaterVM.setProps({ memberInfo: this.props.memberInfo });
          }, 0.1);
        } else {
    ...
    const hotUpdateVersion = UseSetOption.Instance().option.gameSet.hall
      ? UseSetOption.Instance().option.gameSet.hall.hotUpdateVersion
      : "";
    this.propertyNode.props_pc_version.string =
      GameConfig.appLocalVersion +
      (hotUpdateVersion && ".") +
      hotUpdateVersion.replace("v", "").replace(/\./g, "");
    this.propertyNode.props_pc_uniqueId.string = deviceInfo.getUniqueId();
    this.propertyNode.props_Toggle_set_music.getComponent(Toggle).isChecked =
      UseSetOption.Instance().option.music;
    this.propertyNode.props_Toggle_set_sound.getComponent(Toggle).isChecked =
      UseSetOption.Instance().option.sound;
    this.propertyNode.props_Toggle_set_vibration.getComponent(
      Toggle
    ).isChecked = UseSetOption.Instance().option.vibration;
    this.propertyNode.props_label_lang.string =
      defaultLanguageType[config.country].langName;
  }
  ...
  private containerEventHandler(event: Event, customEventData: string) {
    ...
    switch (event.target["name"]) {
      case "Toggle_left_profil":
        tween(
          this.propertyNode.props_Mask_perfabs_left.getComponent(UITransform)
        )
          .to(0.1, { height: 138 })
          .start();
        tween(this.propertyNode.props_Mask_share.getComponent(UITransform))
          .to(0.1, { height: 0 })
          .start();
  ...
  private async openEditNickName() {
    const changeNickNameViewModel = new BaseViewModel<
      Hall_ChangeNickName,
      CNIState,
      CNIprops,
      CNIEvent
    >("Hall_ChangeNickName")
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._PC_CHANGE_NICKNAME,
            Prefab
          )
        ).source
      )
  ...
  private selectToggleProfil(index: number) {
    const nodeName =
      index === 0 ? "Toggle_left_informasi" : "Toggle_left_setting";
    this.propertyNode.props_ToggleGroup_profil.children
      .filter((v) => v.name.startsWith("Toggle_"))
      .forEach((v) => {
        v.getComponent(Toggle).isChecked = v.name === nodeName;
      });
  }
  private selectToggleLeftRansel(nodeName: string) {
    this.propertyNode.props_ToggleGroup_perfabs.children
      .filter((v) => v.name.startsWith("Toggle_"))
      .forEach((v) => {
        v.getComponent(Toggle).isChecked = v.name === nodeName;
      });
    this.propertyNode.props_Mask_perfabs_left.getComponent(UITransform).height =
      nodeName === "Toggle_left_profil" ? 138 : 0;
  }
  ...
  private updateHandler1SelectStatus() {
    const selectNode = this.propertyNode.props_ToggleGroup_profil.children.find(
      (v) => v.getComponent(Toggle).isChecked
    );
  ...
  private containerEventHandler3() {
    hallAudio.playOneShot(SoundPathDefine.BTU_CLICK);
    const selectNode = this.propertyNode.props_ToggleGroup_Share.children.find(
      (v) => v.getComponent(Toggle).isChecked
    );
    if (selectNode.name === "Toggle_left_informasi") {
      this.setState({ profileShowIndex: 4 });
  ...
  private loadRecommendData(isforceRefresh = false) {
    if (!this.share_InfoModel) {
      sourceManageSeletor()
        .getFileAsync(PrefabPathDefine.SHARE_Info, Prefab)
        .then((file) => {
          this.share_InfoModel = new BaseViewModel<
            Share_Info,
            SHIState,
            SHIprops,
            SHIEvent
          >("Share_Info")
            .mountView(file.source)
            .appendTo(this.node)
            .setProps({
              isShow: true
            })
  ...
  private loadBawahanData() {
    if (!this.share_MemberModel) {
      this.share_MemberModel = new BaseViewModel<
        Hall_PC_Share_member,
        MBIState,
        MBIprops,
        MBIEvent
      >("Hall_PC_Share_member")
        .mountView(
          sourceManageSeletor().getFile(PrefabPathDefine.SHARE_MEMBER_LIST)
            .source
        )
        .appendTo(this.node, { effectType: EffectType.EFFECT1, isModal: false })
        .setProps({
          isShow: true
        })
        .setEvent({});
  ...
  private initWebView() {
    ...
    const url =
      config.agentPageUrl + "?token=" + sys.localStorage.getItem("token");
