...
export class Hall_AppDownLoadGuide extends BaseComponent<
  IState,
  IProps,
  IEvent
> {
  ...
  protected bindEvent(): void {
    ...
    this.propertyNode.props_btn_download.node.on(
      Node.EventType.TOUCH_END,
      () => {
        this.gotoDownload();
      }
    );
    this.propertyNode.props_downLoad_btu.node.on(
      Node.EventType.TOUCH_END,
      () => {
  ...
  private downLoadPanelTroggle(isShow: boolean) {
    if (!isShow) {
      const targetNodePosition = getNodePositionInCanvas(
        this.propertyNode.props_downLoad_btu.node
      );
      const { x, y, z } = this.propertyNode.props_spr_guide_bg.position;
      this.downLoadBtuTroggle(true);
      tween(this.propertyNode.props_spr_guide_bg)
        .to(0.2, { position: targetNodePosition, scale: new Vec3(0, 0, 0) })
        .call(() => {
      ...
      tween(this.propertyNode.props_spr_guide_bg)
        .to(0.2, { position: new Vec3(0, 0, 0), scale: new Vec3(1, 1, 1) })
        .start();
    }
  }
  private downLoadBtuTroggle(isShow: boolean) {
    ...
    if (isShow) {
      tween(this.propertyNode.props_downLoad_btu.node)
        .to(0.3, {
          position: new Vec3(
            x +
              this.propertyNode.props_downLoad_btu.node.getComponent(
                UITransform
              ).width,
            y,
            z
  ...
  private gotoDownload() {
    ...
    sys.openURL(
      openUrl ||
        (sys.os === sys.OS.IOS
          ? config.appleAppDumpUrl
          : `${config.appDumpUrl}?pkgId=` + getPackageName())
    );
  }
