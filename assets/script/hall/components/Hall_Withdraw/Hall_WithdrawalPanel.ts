...
export class Hall_WithdrawalPanel extends BaseComponent<
  IState,
  IProps,
  IEvent
> {
  ...
  protected bindEvent(): void {
    ...
    this.propertyNode.props_btn_withdrawal_ikat.on(
      Node.EventType.TOUCH_END,
      () => {
        this.events.openWithdrawBank();
      }
    );
    this.propertyNode.props_btn_withdrawal_subtract.on(
      Node.EventType.TOUCH_END,
      () => {
        const amountStr =
          this.propertyNode.props_EditBox_jumlah.getComponent(EditBox).string;
    ...
    this.propertyNode.props_btn_withdrawal_add.on(
      Node.EventType.TOUCH_END,
      () => {
        const amountStr =
          this.propertyNode.props_EditBox_jumlah.getComponent(EditBox).string;
        ...
        if (amount + 100 > this.props.memberAssetGoldPieces) {
          this.propertyNode.props_EditBox_jumlah.getComponent(EditBox).string =
            this.props.memberAssetGoldPieces + "";
        } else {
          this.propertyNode.props_EditBox_jumlah.getComponent(EditBox).string =
            amount + 10000 + "";
        }
    ...
    this.propertyNode.props_btn_withdrawal_max.on(
      Node.EventType.TOUCH_END,
      () => {
        this.propertyNode.props_EditBox_jumlah.getComponent(EditBox).string =
          this.props.memberAssetGoldPieces + "";
    ...
    this.propertyNode.props_btn_withdrawal_catatan.on(
      Node.EventType.TOUCH_END,
      () => {
        this.events.openWithdrawBill();
      }
    );
    this.propertyNode.props_btn_withdrawal_kirim.on(
      Node.EventType.TOUCH_END,
      () => {
        if (
          this.propertyNode.props_btn_withdrawal_kirim.getComponent(Sprite)
            .grayscale
        )
          return;
        if (this.props.withdrawBankChannelChoose) {
          ...
          new InputValidator()
            .begin()
            .isDecimal(amount2, false)
            .done(() => {
              if (+amount2 > this.props.memberAssetGoldPieces) {
                this.dispatch(
                  addToastAction({
                    content: lang.write(
                      (k) => k.HallModule.HallWithdrawalNoEnough,
                      {},
              ...
              if (
                +amount2 < this.props.withdrawBankChannelChoose.limitDown ||
                +amount2 > this.props.withdrawBankChannelChoose.limitUp
              ) {
                this.dispatch(
                  addToastAction({
                    content: lang.write(
                      (k) => k.HallModule.HallWithdrawalglod,
                      {
          ...
          this.dispatch(
            addToastAction({
              content: lang.write(
                (k) => k.HallModule.HallWithdrawalMessage,
                {},
    ...
    this.propertyNode.props_btn_newHelp.node.on(
      Node.EventType.TOUCH_END,
      () => {
  ...
  protected useProps(key: keyof IProps, value: { pre: any; cur: any }) {
    key === "memberAssetGoldPieces" &&
      (this.propertyNode.props_label_saldo_num.getComponent(Label).string =
        value.cur.formatAmountWithCommas());
    if (key === "withdrawBankChannelChoose") {
      ...
      this.propertyNode.props_had_bank_choose
        .getChildByName("account_number")
        .getComponent(Label).string = maskBankCardNumber(cur.accountNumber, 3);
      ...
      assetManager.loadRemote(
        this.props.withdrawBankChannelChoose.iconMax,
    ...
    if (key === "validFlowingWater") {
      this.isFlowingWater();
      !(this.propertyNode.props_btn_withdrawal_kirim as Node).getComponent(
        Sprite
      ).grayscale && this.beginGuide();
    }
  }
  /**添加流水 */
  public addWithdrawBill(dataList: WithDrawHistoryType[]) {
    ...
    const isFrist: boolean =
      this.propertyNode.props_scrollView_withDraw.getComponent(ScrollView)
    ...
    if (isFrist) {
      ...
      dataList.slice(0, 5).forEach((item) => {
        ...
        this.propertyNode.props_scrollView_withDraw
  ...
  private creatorNodeItem(data: WithDrawHistoryType) {
    ...
    node.active = true;
    node.getChildByName("label_time").getComponent(Label).string =
      data.transferTime;
    node.getChildByName("label_name").getComponent(Label).string =
      data.nickName;
    node.getChildByName("label_menarik").getComponent(Label).string =
      data.orderMoney.formatAmountWithCommas() + " " + data.currency;
    node.getChildByName("label_waktu").getComponent(Label).string = lang.write(
      (k) => k.withdrawal.ElapsedTime,
      { minutes: data.minute, second: data.second },
      { placeStr: "多少分多少秒" }
    );
    return node;
  }
    ...
    if (this.props.validFlowingWater > 0) {
      !ModalBox.Instance().isShow &&
        ModalBox.Instance().show(
          {
            content: lang.write(
              (k) => k.withdrawal.flllowWaterNoLevel,
              {},
  ...
  public beginGuide() {
    if (config.fristLogin.main && !config.fristLogin["hasShow_1"]) {
      ...
      const guide_1 = new Guide(
        this.propertyNode.props_btn_withdrawal_ikat,
        this.propertyNode.props_layout_remind_lakt
      );
      const guide_2 = new Guide(
        this.propertyNode.props_EditBox_jumlah,
        this.propertyNode.props_layout_remind_jumlah
      );
      const guide_3 = new Guide(
        this.propertyNode.props_btn_withdrawal_kirim,
        this.propertyNode.props_layout_remind_kirim
      );
      // this.taskScheduler.stopQueue(false)
      this.taskScheduler2
        .joinQueue(
