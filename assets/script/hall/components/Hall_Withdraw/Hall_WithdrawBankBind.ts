...
export class Hall_WithdrawBankBind extends BaseComponent<
  IState,
  IProps,
  IEvent
> {
  ...
  protected bindEvent(): void {
    ...
    this.propertyNode.props_btn_metode_tentukan.node.on(
      Node.EventType.TOUCH_END,
      () => {
        ...
        if (this.props.hadChooseBankChannl.channelType === 1) {
          ...
          new InputValidator()
            .begin()
            .isLocalBankCard(accountNumber)
            .isNotEmpty(
              realName,
              lang.write((k) => k.InputModule.isNotEmpty) + ":Name"
            )
            .isNotEmpty(
              ifscCode,
              lang.write((k) => k.InputModule.isNotEmpty) + ":IFSC"
            )
            .isNotEmpty(
              branchName,
              lang.write((k) => k.InputModule.isNotEmpty) + ":SubBranch"
            )
            .isAllNumber(accountNumber)
            .done(() => {
              this.events.onBindDone(
                this.props.hadChooseBankChannl,
                accountNumber,
                realName,
                ifscCode,
                branchName
  ...
  protected useProps(key: keyof IProps, value: { pre: any; cur: any }) {
    if (key === "hadChooseBankChannl") {
      this.propertyNode.props_nomor_EditBox.string =
        value.cur.accountNumber || "";
      this.propertyNode.props_EditBox_name.string = value.cur.realName || "";

      this.props.hadChooseBankChannl.iconMax &&
  ...
  public beginGuide() {
    if (config.fristLogin.main && !config.fristLogin["hasShow_3"]) {
      ...
      const guide_1 = new Guide(
        this.propertyNode.props_btn_metode_tentukan.node,
