import { _decorator, assetManager, Button, Component, ImageAsset, instantiate, Label, Node, ScrollView, Sprite, SpriteFrame, sys, ToggleContainer } from 'cc';
...
export class Hall_WithdrawBank extends BaseComponent<IState, IProps, IEvent> {
		...
		this.propertyNode.props_btn_metode_tentukan.on(Node.EventType.TOUCH_END, () => {
			if (this.chooseItem) {
				if (!this.chooseItem.bind && this.props.withdrawBankChannelList.filter(i => i.bind).length >= 1) {
					this.dispatch(addToastAction({ content: lang.write(k => k.HallModule.HallWithdrawalBank, {}, { placeStr: "请选择有效的提现渠道~" }) }))
					return
				}
				this.events.onBankChannlDone(this.chooseItem)
			} else {
				this.dispatch(addToastAction({ content: lang.write(k => k.HallModule.HallWithdrawalBank, {}, { placeStr: "请选择~" }) }))
			}
	...
	protected useProps(key: keyof IProps, value: { pre: any, cur: any }) {
		if (key === "withdrawBankChannelList") {
			...
			value.cur.sort((a, b) => b.bind - a.bind).forEach((item: WithdrawBankChannelType, index) => {
			...
			this.setState({ isDisableBind: this.props.withdrawBankChannelList.filter(i => i.bind).length ? true : false })
			this.beginGuide()
		}
	}
	...
	callback(event: Event, customEventData: string) {
		...
		const index = this.propertyNode.props_ToggleGroup_bankType.children.indexOf(event.target as any)
