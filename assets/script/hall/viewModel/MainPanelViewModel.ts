...
import {
  ...
  IProps as UGIProps,
  IEvent as UGIEvent
} from "../components/Hall_Upgrade";
import {
  Hall_ChangePassword,
  IState as CPIState,
  IProps as CPIProps,
  IEvent as CPIEvent
} from "../components/Hall_ChangePassword";
import {
  Hall_ReliefPanel,
  IState as RPIState,
  IProps as RPIProps,
  IEvent as RPIEvent
} from "../components/Hall_reliefPanel";
...
import {
  ...
  IEvent as WBEvent,
  WinningType
} from "../components/Hall_WinningBox";
import {
  Hall_AppDownLoadGuide,
  IState as ALState,
  IProps as ALProps,
  IEvent as ALEvent
} from "../components/Hall_AppDownLoadGuide";
import {
  getFirebaseToken,
  getPackageName,
  pullPurchaseEvents,
  purchaseAppsflyerEvents
} from "../../common/bridge";
...
class MainPanelViewModel extends ViewModel<Hall_MainPanel, IProps, IEvent> {
  ...
  private winningBoxVM: BaseViewModel<
    Hall_WinningBox,
    WBState,
    WBProps,
    WBEvent
  ...
  protected begin() {
    ...
    !this.isCheckPop &&
      hallWebSocketDriver.sendSktMessage(SKT_MAG_TYPE.POP_UPS, "", {});
    ...
    hallWebSocketDriver.sktMsgListener.addOnce(
      SKT_MAG_TYPE.POP_UPS,
      "main",
      (data, error) => {
        if (error) return;
        ...
        try {
          ...
          const queue = [
            { key: "activityVo", ...activityVo },
            { key: "signInVo", ...signInVo },
            { key: "upgradesVo", ...upgradesVo }
          ].sort((a, b) => a.sort - b.sort);
          queue.forEach((item) => {
            if (item.popUps && item.key === "activityVo") {
              TaskSchedulerDefault().joinQueue(
                new Task((done) =>
                  this.openAwardPanel(false, () => {
                    done();
                  })
                )
              );
              ...
              TaskSchedulerDefault().joinQueue(
                new Task((done) =>
                  this.openUpgradePanel(this.parentNode, () => {
                    done();
                  })
                )
              );
            ...
            if (item.popUps) {
              ...
              this.serviceVm &&
                !this.serviceVm.isUnMount &&
                this.serviceVm.unMount();
            }
          });
        ...
        if (!error && this.comp.props && this.comp.props.subGameInfo) {
          global.closeSubGame({
            confirmContent: lang.write(
              (k) => k.BaseBoardModule.BaseBoardLogin,
              {},
    ...
    hallWebSocketDriver.sktMsgListener.add(
      ...
      "mainboard",
      (data: Array<GiftItemType>, error) => {
        if (error || this.isOpenShop) return;
        this.comp.setGiftButtonDisplay(
          !data.filter((i) => i.separatePage === 1).length ? false : true
        );
    ...
    hallWebSocketDriver.sktMsgListener.add(
      SKT_MAG_TYPE.WINNING_BOX,
      "main",
      (data: WinningType, error) => {
        if (error) {
          console.error(error);
          return;
        }
        ...
        this.winningBoxVM &&
          this.winningBoxVM.comp.pushWinningData(
            data,
            this.comp.props.subGameInfo &&
              this.comp.props.subGameInfo.isVertical
          );
      }
    );

    hallWebSocketDriver.sktMsgListener.add(
      SKT_MAG_TYPE.VIP_CHANGE,
      "main",
      (data, error) => {
        if (error) return;
        this.openVipUp();
    ...
    hallWebSocketDriver.sktMsgListener.add(
      SKT_MAG_TYPE.RECHANGE_CALLBACK,
      "main",
      (data: RechangeCallbackVo, error) => {
        if (error) return;
    ...
    if (!this.winningBoxVM) {
      sourceManageSeletor()
        .getFileAsync(PrefabPathDefine._WINNING_BOX, Prefab)
        .then(({ source }) => {
          this.winningBoxVM = new BaseViewModel<
            Hall_WinningBox,
            WBState,
            WBProps,
            WBEvent
          >("Hall_WinningBox")
            .mountView(source)
            .appendTo(
              baseBoardView.comp.getPropertyNode().props_winnerBox as Node
            );
        });
    }
    this.setEvent({
      ...
      onOpenMailPanel: async () => {
        ...
        const mailBoxViewModel = new MailBoxViewModel()
          .mountView(
            (
              await sourceManageSeletor().getFileAsync(
                PrefabPathDefine._HELL_MAIL,
                Prefab
              )
            ).source
          )
      ...
      onOpenWithdrawalPanel: () => {
        const withdrawalViewModel = new WithdrawalViewModel()
          .mountView(
            sourceManageSeletor().getFile(PrefabPathDefine.HELL_WITHDRAWAL)
              .source
          )
          .appendTo(this.viewNode, {
            effectType: EffectType.EFFECT1,
      ...
      onOpenService: async () => {
        ...
        this.serviceVm = new BaseViewModel<
          Hall_Service,
          SIState,
          SIProps,
          SIEvent
        >("Hall_Service")
          ...
          .bindDoneHandler(() => {
            this.dispatch(
              setLoadingAction({ isShow: false, flagId: "service" })
            );
          })
          .appendTo(this.viewNode, {
            effectType: EffectType.EFFECT1,
    ...
    const isGuide =
      (sys.os === sys.OS.ANDROID && getPackageName() === "web") ||
    ...
    if (isGuide) {
      sourceManageSeletor()
        .getFileAsync(
          sys.os === sys.OS.IOS
            ? PrefabPathDefine._HELL_APP_DOWNLOAD_PANEL_IOS
            : PrefabPathDefine._HELL_APP_DOWNLOAD_PANEL,
          Prefab
        )
        .then((file) => {
          const appDownLoadVM = new BaseViewModel<
            Hall_AppDownLoadGuide,
            ALState,
            ALProps,
            ALEvent
  ...
  private getVipBonus() {
    fetcher
      .send(ApiUrl.GET_VIP_BONUS, { vipLevel: this.comp.props.vipLevel })
      .then(async (rsp) => {
  ...
  private async openBindPhone(nodeWrap?: Node) {
    const bindPhoneViewModel = new BindPhoneViewModel()
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._HELL_BIND_PHONE,
            Prefab
          )
        ).source
      )
  ...
  public async openUpgradePanel(nodeWrap?: Node, closeHandler?: () => void) {
    ...
    const upgradeViewModel = new BaseViewModel<
      Hall_Upgrade,
      UGIState,
      UGIProps,
      UGIEvent
    >("Hall_Upgrade")
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._HELL_UPGRADE,
            Prefab
          )
        ).source
      )
  ...
  public async openChangePassword(nodeWrap?: Node) {
    const changePwdViewModel = new BaseViewModel<
      Hall_ChangePassword,
      CPIState,
      CPIProps,
      CPIEvent
    >("Hall_ChangePassword")
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._HELL_CHANGE_PASSWORD,
            Prefab
          )
        ).source
      )
  ...
  public openShop(
    defaultBuyType: BuyType = BuyType.COIN,
    nodeWrap?: Node,
    closeHandler?: () => void
  ) {
    ...
    this.shopViewModel = new ShopViewModel()
      .mountView(
        sourceManageSeletor().getFile(PrefabPathDefine.HELL_SHOP).source
      )
      .appendTo(nodeWrap || this.parentNode, {
        effectType: EffectType.EFFECT1,
        isModal: true
  ...
  public openPc(index: number = 1, nodeWrap?: Node) {
    this.personCenterViewModel = new PersonCenterViewModel()
      .mountView(
        sourceManageSeletor().getFile(PrefabPathDefine.PC_MAIN_PANEL).source
      )
      .appendTo(nodeWrap || this.parentNode, {
        effectType: EffectType.EFFECT1,
        isModal: true,
   ...
   * @param data 传入的数据，可选
   */
  public async openGiftBoxPanel(
    isDecideShow: boolean,
    closeHandler?: () => void,
    data?: Array<GiftItemType>
  ) {
    ...
    if (
      isDecideShow &&
      data &&
      !data.filter((i) => i.separatePage === 1).length
    ) {
      //如果无数据就不自动弹
      closeHandler && closeHandler();
      return;
    }
    this.giftBoxVM = new BaseViewModel<Hall_GiftBag, GBState, GBProps, GBEvent>(
      "Hall_GiftBag"
    )
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._HELL_GIFT_BAG,
            Prefab
          )
        ).source
      )
  ...
  public async openSign(nodeWrap?: Node, closeHandler?: () => void) {
    ...
    const signInViewModel = new SignInViewModel()
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._HELL_SIGN_IN,
            Prefab
          )
        ).source
      )
  ...
  public async openDailyTaskPanel(nodeWrap?: Node, closeHandler?: () => void) {
    const dailyTaskViewModel = new DailyTaskViewModel()
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._DAILY_TASK,
            Prefab
          )
        ).source
      )
  ...
  private async openVipUp(nodeWrap?: Node) {
    new VipUpViewModel()
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._HELL_VIP_UP,
            Prefab
          )
        ).source
      )
      .appendTo(nodeWrap || this.parentNode, { isModal: true })
      .connect();
  }

  public async openVipMain(nodeWrap?: Node) {
    this.dispatch(setLoadingAction({ isShow: true, flagId: "vip" }));
    this.vipMainVm = new VipMainViewModel()
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            PrefabPathDefine._HELL_VIP_MAIN,
            Prefab
          )
        ).source
      )
  ...
  private onOpenShareHelp(nodeWrap?: Node) {
    ...
    sourceManageSeletor()
      .getFileAsync(PrefabPathDefine._SHARE_HELP, Prefab)
      .then((file) => {
        new ShareHelpViewModel()
          .mountView(file.source)
          .appendTo(nodeWrap || this.parentNode, {
            effectType: EffectType.EFFECT1,
            isModal: true
  ...
  public async openReliefPanel(benefit: number) {
    ...
    if (!this.isOpenBenefit) {
      const reLiefPanel = new BaseViewModel<
        Hall_ReliefPanel,
        RPIState,
        RPIProps,
        RPIEvent
      >("Hall_ReliefPanel")
        .mountView(
          (
            await sourceManageSeletor().getFileAsync(
              PrefabPathDefine._HELL_RE_LIEF,
              Prefab
            )
          ).source
        )
        ...
        .setEvent({
          onClosePanel: () => {
            fetcher
              .send(ApiUrl.BENEFIT, undefined, undefined, undefined, {
                isLoading: false
              })
              .then(async (data) => {
                if (data) {
  ...
  public isTouristPass() {
    if (this.comp.props.tourist) {
      this.dispatch(
        addToastAction({
          content: lang.write(
            (k) => k.HallModule.CantEditProfile,
            {},
  ...
  private onOpenShareSure(
    ...
    if (actingId === promotionCode) {
      this.dispatch(
        addToastAction({
          content: lang.write(
            (k) => k.MainPaneModule.MainPanelCodeError,
            {},
            { placeStr: "不能输入自己的推荐码" }
          ),
    ...
    fetcher
      .send(ApiUrl.RECOMMEND_MEMBER_INFO, { promotionCode: actingId }, "post")
      .then((rsp) => {
        if (!rsp || !rsp.memberId) {
          this.dispatch(
            addToastAction({
              content: lang.write(
                (k) => k.MainPaneModule.MainPanelPromotion,
                {},
                { placeStr: "推荐人不存在" }
              ),
              type: ToastType.NORMAL
        ...
        this.dispatch(changeWebView(false));
        sourceManageSeletor()
          .getFileAsync(PrefabPathDefine._SHARE_SURE, Prefab)
          .then((file) => {
            new ShareSureViewModel()
              .mountView(file.source)
              .appendTo(nodeWrap || this.parentNode, {
                effectType: EffectType.EFFECT1,
                isModal: true
  ...
  private rechangeCallbackHandle(data: RechangeCallbackVo) {
    ...
    fetcher
      .send(ApiUrl.ADJUST_ACCEPT, { id: data.id }, "post", undefined, {
        isThrottler: false
      })
      .then((rsp) => {
    ...
    if (memberId && token) {
      fetcher
        .send(
          ...
          { keys: keys, memberId: memberId, token: token },
          "post"
        )
        .then((rsp) => {
      ...
      this.retryCount < 5 &&
        this.comp.scheduleOnce(() => {
          this.retryCount++;
          console.log(
            "fcm bind firebase token retry get!! retryCount: " + this.retryCount
          );
          this.bingFcmToken();
        }, 5);
    }
