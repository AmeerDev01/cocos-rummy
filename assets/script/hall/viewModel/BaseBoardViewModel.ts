...
import {
  PrefabPathDefine as HallPrefabPathDefine,
  PrefabPathDefine
} from "../../hall/sourceDefine/prefabDefine";
import {
  ToastPosition,
  ToastType,
  addToastAction,
...
import {
  ...
  IProps as GPIProps,
  IEvent as GPIEvent
} from "../components/Hall_GivePanel";
...
class BaseBoardViewModel extends ViewModel<Hall_Baseboard, IProps, IEvent> {
  ...
  protected begin() {
    ...
    hallWebSocketDriver.sktMsgListener.addOnce(
      SKT_MAG_TYPE.GIVE_GLOD,
      "give",
      (data, error) => {
        if (error) return;
        sourceManageSeletor()
          .getFileAsync(PrefabPathDefine._HELL_WELCOME, Prefab)
          .then((file) => {
            const givePanel = new BaseViewModel<
              Hall_GivePanel,
              GPIState,
              GPIProps,
              GPIEvent
  ...
  private initMainPanel(baseBoard) {
    this.mainPanelViewModel = new MainPanelViewModel()
      .mountView(
        sourceManageSeletor().getFile(HallPrefabPathDefine.MAIN_PANEL).source
      )
      .appendTo(baseBoard)
      .setEvent({
          ...
          .catch((e) => {
            !isForce &&
              this.dispatch(
                addToastAction({
                  content: lang.write(
                    (k) => k.BaseBoardModule.BaseBoardExit,
                    {},
                    { placeStr: "退出失败~" }
                  ),
                  forceLandscape: false
  ...
  private async initLoginPanel(baseBoard: Node, isAutoLogin: boolean = true) {
    ...
    this.loginPageV2ViewModel = new LoginPageV2ViewModel()
      .mountView(
        (
          await sourceManageSeletor().getFileAsync(
            HallPrefabPathDefine._LOGIN_PAGE_V2,
            Prefab
          )
        ).source
      )
      ...
      .setEvent({
        onLoginSuccess: () => {
          this.dispatch(
            setLoadingAction({ isShow: true, flagId: "initLogin" })
          );
          //初始化通信
          socketConnect()
            .then(() => {
              ...
              // }
              this.dispatch(
                setLoadingAction({ isShow: false, flagId: "initLogin" })
              );
  ...
  public isAllowOpenSubGame(gameId: number) {
    if (this.comp.props.openGameInfo) {
      ...
      this.dispatch(
        setSubGameInfoAction(subGameList.find((i) => i.gameId === gameId))
      );
      return true;
    }
  }
    ...
    this.inject(
      ...
      (state: StateType) => {
        ...
        if (state.baseBoard.subGameInfo === null) {
          this.mainPanelViewModel &&
            (this.mainPanelViewModel.viewNode.active = true);
        }
        return {
  ...
  private initInvite() {
    if (!NATIVE) {
      ...
      if (invite) {
        ...
        const channelPackageUrl =
          config.httpBaseUrl + ApiUrl.CHANNEL_PACKAGE + "?number=" + invite;
        fetch(channelPackageUrl)
          .then((response) => {
            response.json().then((data) => {
              if (data.content) {
                ...
                if (data.content.firstPurchase) {
                  adjustWebEventConfig.firstPurchase =
                    data.content.firstPurchase;
                }
                if (data.content.purchase) {
                  adjustWebEventConfig.purchase = data.content.purchase;
                }
                if (data.content.firstPullPurchase) {
                  adjustWebEventConfig.firstPullPurchase =
                    data.content.firstPullPurchase;
                }
