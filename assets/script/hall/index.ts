...
export const sourceManageSeletor = (bundleName: string = "hall") =>
          ...
          const loaderviweModel = new LoaderPanelViewModel()
            ...
            .setEvent({
              onLoadDone: (_sourceManageMap) => {
                ...
                !hallAudio &&
                  (hallAudio = new AudioMgr<SoundPathDefine>(
                    sourceManageSeletor("hall")
                  ));
                !commonAudio &&
                  (commonAudio = new AudioMgr<CommonSoundPathDefine>(
                    sourceManageSeletor("common")
                  ));
                loaderviweModel.unMount().then(() => {
                  baseBoardView = new BaseBoardViewModel()
                    .mountView(
                      sourceManageSeletor("hall").getSourceFile(
                        HallPrefabPathDefine.BASE_BOARD
                      )
                    )
                    .appendTo(rootNode);
...
const initHallGlobal = () => {
  window.HALL_GLOBAL.hallDispatch = (action: AnyAction) => {
  ...
  window.HALL_GLOBAL.openVipMain = () => {
    baseBoardView &&
      baseBoardView &&
      baseBoardView.mainPanelViewModel.openVipMain();
  };
  ...
  window.HALL_GLOBAL.loadHeadSprite = (icon: number, sprite: Sprite) => {
    bundleCommon &&
      bundleCommon.load(
        `resource/head/head_circle_${icon}/spriteFrame`,
        SpriteFrame,
        (err, sp) => {
          if (!err) {
  ...
  window.HALL_GLOBAL.showGiftWindow = (
    gameType: GameType,
  ...
  window.HALL_GLOBAL.setSubGameRunState = (
