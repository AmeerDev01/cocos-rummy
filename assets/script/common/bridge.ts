 ...
 * @param first 是否首次
 */
export const purchaseAppsflyerEvents = (
  revenue: string,
  currency: string,
  first: boolean,
  id: string
  ...
  if (NATIVE && Number(revenue) > 0) {
    ...
    const eventToken = first
      ? adjustWebEventConfig.firstPurchase
      : adjustWebEventConfig.purchase;
    callAndroid(
      first ? WebEventType.FIRST_PURCHASE : WebEventType.PURCHASE,
      eventToken,
      revenue,
      currency,
      id
...
/**充值成功发送事件 */
export const purchaseAdjustEvents = (
  revenue: string,
  currency: string,
  first: boolean,
  id: string
...
/**拉取充值事件 */
export const pullPurchaseEvents = (
  revenue: string,
  currency: string,
  first: boolean,
  id: string
  ...
  if (NATIVE && Number(revenue) > 0) {
    ...
    const eventToken = first
      ...
      : adjustWebEventConfig.pullPurchase;
    callAndroid(
      first ? WebEventType.FIRST_PULL_PURCHASE : WebEventType.PULL_PURCHASE,
      eventToken,
      revenue,
      currency,
      id
...
/**appsflyer拉取充值事件 */
const pullPurchaseAppsflyerEvents = (
  revenue: string,
  currency: string,
  first: boolean,
  id: string
 ...
 * @param first
 */
const pullPurchaseAdjustEvents = (
  revenue: string,
  currency: string,
  first: boolean,
  id: string
...
export const callAndroid = (
  ...
  if (typeof window.Android !== "undefined" && window.Android !== null) {
    ...
    console.log(
      `Executed successfully, event type: ${type}, eventToken: ${eventToken}`
    );
  } else {
...
export const facebookWebEveht = (type, ...values) => {
  ...
  if (typeof window.fbq !== "undefined" && window.fbq !== null) {
    if (type === WebEventType.REGISTER) {
      ...
      window.fbq("track", "Purchase_success", {
        currency: values[1],
        value: values[0]
      });
      ...
      window.fbq("track", "InitiateCheckout_success", {
        currency: values[1],
        value: values[0]
      });
    }
