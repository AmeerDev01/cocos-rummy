...
import {
  ...
  IEvent,
  GIFT_ICON_CONFIG
} from "../components/Common_GiftUser";
...
class GiftUserViewModel extends ViewModel<Common_GiftUser, IProps, IEvent> {
  ...
  public static show(
    ...
    callback: (value: number, memberId?: string) => void
  ) {
    if (initStatus) {
      return;
    }
    ...
    if (this.instance) {
      ...
      this.instance.viewNode.setSiblingIndex(
        this.instance.viewNode.parent.children.length
      );
      initStatus = false;
    } else {
      this.instance = new GiftUserViewModel()
        .mountView(
          sourceManageSeletor("common").getFile(PrefabPathDefine.GIFT_USER)
            .source
        )
        .appendTo(parent);
    }
  ...
  public static flyGift(
    ...
    const sourceFile = sourceManageSeletor("common").getFile(
      PrefabPathDefine.GIFT_ANIMATION
    );
    if (!sourceFile) {
    ...
    const sprGiftNode = node
      .getChildByName("props_spr_gift")
      .getComponent(Sprite);
    sprGiftNode.spriteFrame = sourceManageSeletor("common").getFile(
      iconConfig.spriteFramePath
    ).source;
    ...
    const source = sourceManageSeletor("common").getFile(
      iconConfig.skeletonPath
    ).source;
    ...
    tween(node)
      .to(0.5, { worldPosition: endPos, angle: iconConfig.angle })
      .call(() => {
        if (node.isValid) {
