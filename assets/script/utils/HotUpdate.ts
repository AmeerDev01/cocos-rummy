  ...
  _hotUpdateCallBack(event: native.EventAssetsManager) {
    ...
    switch (code) {
      case native.EventAssetsManager.ALREADY_UP_TO_DATE:
        console.log(
          "It is the same as the remote version and does not need to be updated"
        );
      ...
      case native.EventAssetsManager.NEW_VERSION_FOUND:
        ...
        this._options.OnNeedToUpdate &&
          this._options.OnNeedToUpdate(event.getTotalBytes());
        break;
      ...
      case native.EventAssetsManager.ERROR_UPDATING:
        console.log(
          "ERROR_UPDATING Asset update error: " +
            event.getAssetId() +
            ", " +
            event.getMessage()
        );
  ...
  init(customManifest: native.Manifest, storagePath: string, opt: HotOptions) {
    ...
    if (curVerStr) {
      ...
      if (previousVersion < GameConfig.HotUpdateVersion) {
        ...
        sys.localStorage.setItem(
          "currentVersion",
          String(GameConfig.HotUpdateVersion)
        );
        game.restart();
      }
    } else {
    ...
    this._assetsMgr = new native.AssetsManager(
      ...
      (versionA, versionB) => {
        ...
        console.log(
          "Client Version: " + versionA + ", The Latest Version: " + versionB
        );
        UseSetOption.Instance().setGameOption("hall", {
