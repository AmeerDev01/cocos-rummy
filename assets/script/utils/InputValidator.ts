...
export default class InputValidator {
  ...
  begin(defaultchar: string = "") {
    ...
    this.errorinfo = [];
    this.defaultchar =
      typeof defaultchar === "string" ? defaultchar.trim() : defaultchar;
    return this;
  }
  isNotEmpty(char?: string, customErrStr?: string) {
    ...
    if (!(char || this.defaultchar).length) {
      this.isPass = false;
      this.errorinfo.push(
        customErrStr || lang.write((k) => k.InputModule.isNotEmpty)
      );
    }
    return this;
  }
  isAllNumber(char?: string, customErrStr?: string) {
    if (!this.isPass) return this;
    const isPass = /^\d+$/.test(
      (typeof char === "string" ? char.trim() : char) || this.defaultchar
    );
    if (!isPass) {
      this.isPass = false;
      this.errorinfo.push(
        customErrStr || lang.write((k) => k.InputModule.isAllNumber)
      );
    }
    return this;
  }
  isDecimal(
    char?: string,
    isAllowFloat: boolean = true,
    customErrStr?: string
  ) {
    if (!this.isPass) return this;
    let isPass = /^\d+(\.\d{1,2})?$/.test(
      (typeof char === "string" ? char.trim() : char) || this.defaultchar
    );
    if (!isAllowFloat) {
      isPass = /^[1-9]\d{0,15}$/.test(
        (typeof char === "string" ? char.trim() : char) || this.defaultchar
      );
    }
    if (!isPass) {
      this.isPass = false;
      this.errorinfo.push(
        customErrStr || lang.write((k) => k.InputModule.isDecimal)
      );
    }
    return this;
  }
  isPhoneNumber(char?: string, customErrStr?: string) {
    if (!this.isPass) return this;
    const isPass = /^\d{11,13}$/.test(
      (typeof char === "string" ? char.trim() : char) || this.defaultchar
    );
    if (!isPass) {
      this.isPass = false;
      this.errorinfo.push(
        customErrStr || lang.write((k) => k.InputModule.isPhoneNumber)
      );
    }
    return this;
  }
  ...
  isIDAPhoneNumber(char?: string, customErrStr?: string) {
    if (!this.isPass) return this;
    const isPass = /^8\d{9,11}$/.test(
      (typeof char === "string" ? char.trim() : char) || this.defaultchar
    );
    if (!isPass) {
      this.isPass = false;
      this.errorinfo.push(
        customErrStr || lang.write((k) => k.InputModule.isIDAPhoneNumber)
      );
    }
    return this;
  }
  ...
  isIDPhoneNumber(char?: string, customErrStr?: string) {
    ...
    const isPass = /^\d{10}$/.test(
      (typeof char === "string" ? char.trim() : char) || this.defaultchar
    );
    if (!isPass) {
      this.isPass = false;
      this.errorinfo.push(
        customErrStr || lang.write((k) => k.InputModule.isIDPhoneNumber)
      );
    }
    return this;
  }
  ...
  isSmsCode(char?: string, customErrStr?: string) {
    if (!this.isPass) return this;
    const isPass = /^\d{6}$/.test(
      (typeof char === "string" ? char.trim() : char) || this.defaultchar
    );
    if (!isPass) {
      this.isPass = false;
      this.errorinfo.push(
        customErrStr || lang.write((k) => k.InputModule.isSmsCode)
      );
    }
    return this;
  }
  isCharLength(length: [number, number], char?: string, customErrStr?: string) {
    ...
    if (!isPass) {
      ...
      this.errorinfo.push(
        customErrStr ||
          lang.write((k) => k.InputModule.isCharLength, {
            num1: length[0],
            num2: length[1]
          })
      );
  ...
  /**展示错误信息 */
    ...
    passHandler: (validator: InputValidator) => void,
    ...
    if (this.isPass) {
      ...
      getStore().dispatch(
        addToastAction({ content: this.errorinfo.join(",") })
      );
    }
  }
}
