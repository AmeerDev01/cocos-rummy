...
const envConfig = {
  development: ["https://sg-e-prod.s3.ap-southeast-1.amazonaws.com/jsonConfig-test.json"],
  test: ["https://sg-e-prod.s3.ap-southeast-1.amazonaws.com/jsonConfig-test.json"],
...
export const getIsTest = () => {
  ...
  const isDev = DEV || pkgCode.includes('test') || pkgCode.includes('development') || pkgCode.includes('advance')// || (!DEV && pkgCode.includes('web'))
...
/**是否是生产环境H5版 */
export const isH5 = () => !getIsTest() && getPackageName() === 'web' && window['installBundle']
