...
export class DownloadManagerGame {
  ...
  private SetVerifyCallback(
    path: string,
    asset: native.ManifestAsset
  ): boolean {
  ...
  private GetModulePath(): string {
    return (
      native.fileUtils.getWritablePath() + "remote/" + this.currentModule + "/"
    );
  }
  ...
  private GetModuleManifestUrl(): string {
    ...
    console.log(
      ...
      "assets/" + this.currentModule + "/project.manifest"
    );
    ...
    this.BatchDownload(
      (downloaded: number, total: number) => {
        console.log(
          ...
          downloaded,
          total,
          this.downloadList[this.downloadIndex]
        );
        progressCallback &&
          progressCallback(
            downloaded,
      ...
      () => {
        ...
        this.downloadList = [];
        compeleteCallback &&
          compeleteCallback(this.downloadList[this.downloadIndex]);
      },
      moduleCallback
    );
  }
  ...
  public Download(
    ...
    if (!this.jsbAssetManager) {
      ...
      this.jsbAssetManager = new native.AssetsManager(
        manifestUrl,
        storagePath,
        this.CompareVersion.bind(this)
      );
  ...
  private CheckDownload(): void {
    ...
    if (
      this.jsbAssetManager.getState() === native.AssetsManager.State.UNINITED
    ) {
    ...
    if (
      !this.jsbAssetManager.getLocalManifest() ||
      !this.jsbAssetManager.getLocalManifest().isLoaded()
    ) {
      console.log("Failed to load local manifest ...");
      var manifest = new native.Manifest(
        this.MakeDefaultManifest(),
        this.GetModulePath()
      );
    ...
    this.jsbAssetManager.setEventCallback(
      this.CheckDownloadCallback.bind(this)
    );
      ...
      case native.EventAssetsManager.NEW_VERSION_FOUND:
        ...
        console.log(
          "New version found, please try to update. (" +
            this.jsbAssetManager.getTotalBytes() +
            ")"
        );
        break;
      default:
  ...
  private StartDownload(): void {
    if (this.jsbAssetManager && !this.isDownloading) {
      ...
      if (
        ...
        console.log(
          "@@@@@@@@URL_____StartDownload",
          native.fileUtils.isFileExist(url)
        );
        this.jsbAssetManager.loadLocalManifest(url);
      }
  ...
  private DownloadCallback(event: native.EventAssetsManager): void {
    ...
    switch (eventCode) {
      ...
      case native.EventAssetsManager.UPDATE_FAILED:
        ...
        this.failCallback &&
          this.failCallback("Update failed. " + event.getMessage());
        break;
